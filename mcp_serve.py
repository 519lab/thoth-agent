"""
Thoth MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Thoth-specific extra)

Usage:
    thoth mcp serve
    thoth mcp serve --verbose
...
logger = logging.getLogger("thoth.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using THOTH_HOME."""
    try:
        from thoth_constants import get_thoth_home
        return get_thoth_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("THOTH_HOME", Path.home() / ".thoth")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("THOTH_HOME", Path.home() / ".thoth")
...
class EventBridge:
    ...
    This is the Thoth equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        for session_key, entry in entries.items():
            ...
            try:
                ...
                messages = _thoth_db.run_sync(db.get_messages(session_id))
            except Exception:
                continue
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Thoth MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "thoth",
        instructions=(
            "Thoth Agent messaging bridge. Use these tools to interact with "
    ...
    def messages_read(
        ...
        try:
            import thoth_db as _thoth_db
            all_messages = _thoth_db.run_sync(db.get_messages(session_id))
        except Exception as e:
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Thoth MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(

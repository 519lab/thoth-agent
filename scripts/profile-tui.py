#!/usr/bin/env python3
"""Drive the Thoth TUI under THOTH_DEV_PERF and summarize the pipeline.
...
bypasses the thoth_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.
...
or produced no perf data (suggests THOTH_DEV_PERF wiring is broken).
"""
...
DEFAULT_TUI_DIR = Path(
    os.environ.get("THOTH_TUI_DIR")
    or str(Path(__file__).resolve().parent.parent / "ui-tui")
)
DEFAULT_LOG = Path(os.environ.get("THOTH_PERF_LOG", str(get_thoth_home() / "perf.log")))
DEFAULT_STATE_DB = get_thoth_home() / "state.db"
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — THOTH_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["THOTH_DEV_PERF"] = "1"
    env["THOTH_DEV_PERF_MS"] = str(args.threshold_ms)
    env["THOTH_DEV_PERF_LOG"] = str(log)
    env["THOTH_TUI_RESUME"] = sid
    ...
    node = os.environ.get("THOTH_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="THOTH_DEV_PERF_MS (0 = capture all)")

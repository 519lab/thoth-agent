...
VERSION_FILE = REPO_ROOT / "thoth_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (THOTH_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (THOTH_HOME inject)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"thoth-agent[acp]=={semver}"
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/519lab/thoth-agent",
    ...
    lines.append(f"# Thoth Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Thoth Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Thoth Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Thoth Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Thoth Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Thoth Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Thoth Agent v{new_version} ({calver_date})' "

#!/usr/bin/env bash
# Canonical test runner for thoth-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.thoth/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.thoth/app/venv" "$HOME/.thoth/thoth-agent/venv"; do
...
if [ -f "$HOME/.thoth/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.thoth"

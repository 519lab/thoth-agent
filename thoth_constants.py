"""Shared constants for Thoth Agent.
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Thoth is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``THOTH_OPTIONAL_SKILLS``.
    """
    override = os.getenv("THOTH_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    """
    override = os.getenv("THOTH_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)

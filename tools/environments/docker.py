...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``THOTH_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("THOTH_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using THOTH_DOCKER_BINARY override: %s", override)
        return override
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        container_name = f"thoth-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _THOTH_PROVIDER_ENV_BLOCKLIST)
        thoth_env = _load_thoth_env_vars() if forward_keys else {}

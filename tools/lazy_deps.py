"""
Lazy dependency installer for opt-in Thoth Agent backends.

Many Thoth features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``thoth-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``thoth tools`` or the manual pip command.
...
def _allow_lazy_installs() -> bool:
    ...
    """
    if os.environ.get("THOTH_DISABLE_LAZY_INSTALLS") == "1":
        return False
    try:
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``thoth update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    Mirrors the strategy in ``thoth_cli.tools_config._pip_install`` but
...
def active_features() -> list[str]:
    ...
    Used by ``thoth update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``thoth update``. Never raises; lazy-install failures

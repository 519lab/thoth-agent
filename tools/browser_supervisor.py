...
One ``CDPSupervisor`` runs per Thoth ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "thoth-dialog-bridge.invalid"
...
(() => {
  if (window.__thothDialogBridgeInstalled) return;
  window.__thothDialogBridgeInstalled = true;
  const ENDPOINT = "http://thoth-dialog-bridge.invalid/";

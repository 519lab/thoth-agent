...
# All skills live in ~/.thoth/skills/ (seeded from bundled skills/ on install).
...
def _is_gateway_surface() -> bool:
    if env_var_enabled("THOTH_GATEWAY_SESSION"):
        return True
    from gateway.session_context import get_session_env
    return bool(get_session_env("THOTH_SESSION_PLATFORM"))
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.thoth/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``THOTH_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
        return get_session_env("THOTH_SESSION_PLATFORM") or ""
    except Exception:
        return ""
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``THOTH_PLATFORM`` environment variable
    3. ``THOTH_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from thoth_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("THOTH_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.thoth/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``thoth skills`` config UI). Default False
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from thoth_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: thoth plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.thoth/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `thoth skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        related_skills = _parse_tags(
            thoth_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )

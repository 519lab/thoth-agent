...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("THOTH_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("THOTH_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set THOTH_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind THOTH_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds THOTH_SESSION_PLATFORM via
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("THOTH_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    if is_gateway or env_var_enabled("THOTH_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    approval_mode = _get_approval_mode()
    if is_truthy_value(os.getenv("THOTH_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `thoth skills browse` (labeled "official") and install it with `thoth skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry such as [agentskills.io](https://agentskills.io). Users can install it with `thoth skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.thoth/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(thoth_home, config)` for setup-wizard integration
...
- Integrate with `thoth memory setup` via `post_setup()` — no need to touch core code
...
echo "OPENROUTER_API_KEY=***" >> ~/.thoth/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/thoth" ~/.local/bin/thoth
...
├── cli.py                    # ThothCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (thoth-cli, thoth-telegram, etc.)
├── thoth_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── thoth_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.thoth/skills/ on install)
...
| Path | Purpose |
|------|---------|
| `~/.thoth/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.thoth/.env` | API keys and secrets |
| `~/.thoth/auth.json` | OAuth credentials (Nous Portal) |
| `~/.thoth/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.thoth/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.thoth/state.db` | SQLite session database |
| `~/.thoth/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.thoth/cron/` | Scheduled job data |
| `~/.thoth/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`thoth_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.thoth/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
(for example `_THOTH_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.thoth`:
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `thoth setup` or update `~/.thoth/.env` locally.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Thoth Agent" is the secondary collaborator. If the contributor's commit shows "Thoth Agent" as author because they used Thoth to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **Test it.** Run `thoth --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.
...
Add to `_BUILTIN_SKINS` dict in `thoth_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `thoth_cli/skin_engine.py` for the full schema and existing skins as examples.
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the thoth wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `thoth_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `thoth_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `thoth_cli/gateway_windows.py::
...
2. **Test manually**: Run `thoth` and exercise the code path you changed
...
- Include: OS, Python version, Thoth version (`thoth version`), full error traceback

...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if thoth_home_str:
            takeover_path = Path(thoth_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(thoth_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded thoth-agent but never re-ran ``thoth setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../thoth-gateway.service"
    ...
    # that's the common case for thoth.
    ...
    # constant in thoth_cli/gateway.py.
    headroom = 30.0

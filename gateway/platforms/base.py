...
# Audio file extensions Thoth recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Thoth's DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Thoth-created Telegram private-chat
...
AUDIO_CACHE_DIR = get_thoth_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_thoth_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_thoth_dir("cache/documents", "document_cache")
...
class BasePlatformAdapter(ABC):
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from thoth_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          THOTH_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          THOTH_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          THOTH_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("THOTH_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("THOTH_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("THOTH_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _thoth_run_generation is set on

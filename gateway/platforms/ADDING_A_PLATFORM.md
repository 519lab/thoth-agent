...
There are two ways to add a platform to the Thoth gateway:
...
**zero changes to core Thoth code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `thoth gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `thoth_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Thoth core.
...
## 12. Status Display (`thoth_cli/status.py`)
...
## 13. Gateway Setup Wizard (`thoth_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ thoth_cli/ toolsets.py \
  --include="*.py" -l | sort -u

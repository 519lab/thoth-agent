/**
 * Thoth Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.thoth/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__THOTH_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "thoth-kanban-dot-triage",
    todo: "thoth-kanban-dot-todo",
    ready: "thoth-kanban-dot-ready",
    running: "thoth-kanban-dot-running",
    blocked: "thoth-kanban-dot-blocked",
    done: "thoth-kanban-dot-done",
    archived: "thoth-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-thoth-task";
  ...
  const LS_BOARD_KEY = "thoth.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="thoth-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "thoth-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "thoth-kanban-md",
  ...
  // standard `drop` event and our `thoth-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("thoth-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("thoth-kanban-column--drop");
          if (target) target.classList.add("thoth-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("thoth-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("thoth-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("thoth-kanban:drop", {
  ...
  function KanbanPage() {
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "thoth-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "thoth-kanban-attention",
        "thoth-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "thoth-kanban-attention-bar" },
        h("span", { className: "thoth-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "thoth-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "thoth-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "thoth-kanban-attention-row",
                  "thoth-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "thoth-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "thoth-kanban-attention-row-id" }, task.id),
                h("span", { className: "thoth-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "thoth-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "thoth-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "thoth-kanban-diag-action-btn",
      action.suggested ? "thoth-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " thoth-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".thoth-kanban-drawer-comment-row input, .thoth-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "thoth-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("thoth-kanban-diag", sevClass) },
      h("div", { className: "thoth-kanban-diag-header" },
        h("span", { className: "thoth-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "thoth-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "thoth-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "thoth-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "thoth-kanban-diag-data-row" },
                  h("span", { className: "thoth-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "thoth-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "thoth-kanban-diag-data-row" },
                h("span", { className: "thoth-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "thoth-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "thoth-kanban-diag-reassign-row" },
            h("span", { className: "thoth-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "thoth-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "thoth-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "thoth-kanban-diag-msg",
              msg.ok ? "thoth-kanban-diag-msg--ok" : "thoth-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "thoth-kanban-section" },
      h("div", { className: "thoth-kanban-section-head-row" },
        h("span", { className: "thoth-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "thoth-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "thoth-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "thoth-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "thoth-kanban-docs-link",
      title: "Open Thoth Kanban docs in a new tab",
      "aria-label": "Thoth Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `thoth kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "thoth-kanban-msg-ok" : "thoth-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "thoth-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "thoth-kanban-boardswitcher" },
      h("div", { className: "thoth-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "thoth-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "thoth-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "thoth-kanban-dialog-title" },
        ...
        h("div", { className: "thoth-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Thoth profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "thoth-kanban-bulk" },
      h("span", { className: "thoth-kanban-bulk-count" },
      ...
      h("div", { className: "thoth-kanban-bulk-priority",
      ...
      h("div", { className: "thoth-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Thoth profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "thoth-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("thoth-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("thoth-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "thoth-kanban-trash",
        dragOver ? "thoth-kanban-trash--drop" : "",
        props.draggingTaskId ? "thoth-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "thoth-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "thoth-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".thoth-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "thoth-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("thoth-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("thoth-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "thoth-kanban-column",
        dragOver ? "thoth-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "thoth-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "thoth-kanban-col-check",
        ...
        h("span", { className: cn("thoth-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "thoth-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "thoth-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "thoth-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "thoth-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "thoth-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "thoth-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "thoth-kanban-lane" },
                  h("div", { className: "thoth-kanban-lane-head" },
                    h("span", { className: "thoth-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "thoth-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "thoth-kanban-card--stale-red";
    if (age >= tier.amber) return "thoth-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".thoth-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "thoth-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "thoth-kanban-card",
        props.selected ? "thoth-kanban-card--selected" : "",
        props.failed ? "thoth-kanban-card--failed" : "",
        props.draggingSource ? "thoth-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "thoth-kanban-card-content" },
          h("div", { className: "thoth-kanban-card-row" },
            h("label", {
              className: "thoth-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "thoth-kanban-card-check",
            ...
            h("span", { className: "thoth-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or thoth kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "thoth-kanban-warning-badge",
                    "thoth-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "thoth-kanban-needs-assignee",
          ...
          h("div", { className: "thoth-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "thoth-kanban-card-row thoth-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "thoth-kanban-assignee",
                            title: `Assigned to Thoth profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "thoth-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "thoth-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "thoth-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "thoth-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Thoth profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Thoth profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "thoth-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "thoth-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "thoth-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "thoth-kanban-drawer-close",
        ...
        data ? h("div", { className: "thoth-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "thoth-kanban-drawer-body" },
      h("div", { className: "thoth-kanban-drawer-title" },
        h("span", { className: cn("thoth-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "thoth-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "thoth-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "thoth-kanban-section" },
        h("div", { className: "thoth-kanban-section-head" }, tx(i18n, "result", "Result")),
        h(MarkdownBlock, { source: t.result, enabled: props.renderMarkdown }),
      ) : null,
      h("div", { className: "thoth-kanban-section" },
        h("div", { className: "thoth-kanban-section-head" },
          `${tx(i18n, "comments", "Comments")} (${comments.length})`),
        comments.length === 0
          ? h("div", { className: "text-xs text-muted-foreground" },
              tx(i18n, "noComments", "— no comments —"))
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "thoth-kanban-comment" },
                h("div", { className: "thoth-kanban-comment-head" },
                  h("span", { className: "thoth-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "thoth-kanban-comment-ago" },
      ...
      h("div", { className: "thoth-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "thoth-kanban-event",
              isDiag ? "thoth-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "thoth-kanban-event-header" },
                  h("span", { className: "thoth-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "thoth-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "thoth-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "thoth-kanban-event-header-plain" },
                  h("span", { className: "thoth-kanban-event-kind" }, e.kind),
                  h("span", { className: "thoth-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "thoth-kanban-event-phantom-row" },
                  h("span", { className: "thoth-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "thoth-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "thoth-kanban-event-payload" },
    ...
    return h("div", { className: "thoth-kanban-section" },
      h("div", { className: "thoth-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "thoth-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `thoth-kanban-run--${r.outcome || r.status || "ended"}`
          : "thoth-kanban-run--active";
        return h("div", { key: r.id, className: cn("thoth-kanban-run", outcomeClass) },
          h("div", { className: "thoth-kanban-run-head" },
            h("span", { className: "thoth-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "thoth-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "thoth-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "thoth-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "thoth-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "thoth-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "thoth-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "thoth-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "thoth-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "thoth-kanban-pre thoth-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "thoth-kanban-meta-row" },
      h("span", { className: "thoth-kanban-meta-label" }, props.label),
      h("span", { className: "thoth-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "thoth-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "thoth-kanban-meta-row" },
        h("span", { className: "thoth-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "thoth-kanban-meta-value thoth-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "thoth-kanban-meta-row" },
        h("span", { className: "thoth-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "thoth-kanban-meta-value thoth-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "thoth-kanban-section" },
      h("div", { className: "thoth-kanban-section-head-row" },
        h("span", { className: "thoth-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "thoth-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "thoth-kanban-section" },
      h("div", { className: "thoth-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "thoth-kanban-deps-row" },
        h("span", { className: "thoth-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "thoth-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "thoth-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "thoth-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "thoth-kanban-dep-chip-x",
      ...
      h("div", { className: "thoth-kanban-deps-row" },
        h("span", { className: "thoth-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "thoth-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "thoth-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "thoth-kanban-section" },
      ...
      h("div", { className: "thoth-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "thoth-kanban-home-sub thoth-kanban-home-sub--on"
              : "thoth-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__THOTH_PLUGINS__ && typeof window.__THOTH_PLUGINS__.register === "function") {
    window.__THOTH_PLUGINS__.register("kanban", KanbanPage);
  }
})();

...
``thoth_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
the in-browser PTY bridge in ``thoth_cli/web_server.py``.

This means ``thoth dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
from thoth_cli import kanban_db
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from thoth_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``thoth_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from thoth_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from thoth_cli.kanban_diagnostics import SEVERITY_ORDER
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from thoth_cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``thoth kanban
...
# spawn failures, stuck-blocked). See thoth_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``thoth kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``thoth kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``thoth kanban specify <task_id>``.
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``thoth kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.thoth/config.yaml.
...
def _active_profile_name() -> str:
    """Return the current Thoth profile name for notify-sub ownership."""
    try:
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.thoth/profiles/*`` on disk and every
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            ...
            profile_dir = _Path(get_thoth_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``thoth profile describe <name> --auto``. Non-OK
    ...
    """
    try:
        from thoth_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    1:1 to ``thoth kanban decompose <task_id>``.
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.thoth/config.yaml.
...
async def stream_events(ws: WebSocket):
    ...
    # authenticates in thoth_cli/web_server.py.

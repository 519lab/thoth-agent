...
>
> When Thoth is installed via `pip install thoth-agent` or cloned from source, this plugin auto-registers as a dashboard tab on first `thoth dashboard` launch. No separate install step. See [Built-in Plugins → hermes-achievements](../../website/docs/user-guide/features/built-in-plugins.md) in the main docs.

Achievement system for the Thoth Dashboard: collectible, tiered badges generated from real local Thoth session history.

![Thoth Achievements dashboard](docs/assets/achievements-dashboard-hd.png)

The screenshots use temporary demo tier data to show the full visual range. The plugin itself reads real local Thoth session history by default.
...
>
> **Share cards (2026-05-04, vendored in thoth-agent v0.4.0):** Unlocked achievement cards now have a "Share" button that renders a 1200×630 PNG share card (client-side canvas, no backend, no network) with Download + Copy-to-clipboard actions. Fits X/Twitter, Discord, LinkedIn, Bluesky link-preview dimensions.

## What it does

Thoth Achievements scans local Thoth sessions and unlocks badges based on real agent behavior:

- autonomous tool chains
- debugging and recovery patterns
- vibe-coding file edits
- Thoth-native skills, memory, cron, and plugin usage
...
- **Secret** — hidden until Thoth detects the first related signal
...
If backend API routes 404, restart `thoth dashboard`; plugin APIs are mounted at dashboard startup.
...
If the update changes backend routes or `plugin_api.py`, restart `thoth dashboard` after pulling.
...
Achievement unlock state is stored locally in `state.json` and is not overwritten by git updates. New achievements are evaluated from your existing Thoth session history. Achievement IDs are stable and should not be renamed casually because they are the unlock-state keys.

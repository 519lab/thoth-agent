...
# ``model_tools`` import time, so ``thoth status``, ``thoth chat``, etc.
...
#     fresh "Thoth is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a thoth MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${THOTH_HOME}/google_chat_thread_counts.json so
        ...
        # they don't sit in the chat forever as "Thoth is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("THOTH_HOME", str(_Path.home() / ".thoth"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${THOTH_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a thoth MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Thoth is thinking…' marker message.
        ...
        "Thoth is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Thoth is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Thoth is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Thoth is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Thoth is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``thoth setup``.
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. thoth-chat-events) and a Pull subscription.")
...
def register(ctx) -> None:
    """Plugin entry point — called by the Thoth plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'thoth-agent[google_chat]'",
        ...
        # the "Thoth is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Thoth is thinking…' marker message "

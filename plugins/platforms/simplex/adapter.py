"""SimpleX Chat platform adapter (Thoth plugin).
...
This adapter ships as a Thoth platform plugin under
``plugins/platforms/simplex/``. The Thoth plugin loader scans the
...
discoverable and `thoth setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Thoth — no
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `thoth gateway status` without

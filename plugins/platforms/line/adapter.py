"""
LINE Messaging API platform adapter for Thoth Agent.
...
to Thoth Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``THOTH_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            thoth_home = Path.home().joinpath(".thoth").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``thoth status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``thoth status`` reflect a LINE configuration that lives entirely

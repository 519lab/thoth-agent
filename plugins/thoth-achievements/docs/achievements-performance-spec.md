# Thoth Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: thoth-achievements plugin
...
### `GET /api/plugins/thoth-achievements/achievements`
...
### `POST /api/plugins/thoth-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/thoth-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/thoth-achievements/overview`
...
- Persist plugin data under:
  - `~/.thoth/plugins/thoth-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.thoth/plugins/thoth-achievements/scan_checkpoint.json`
...
- `~/.thoth/plugins/thoth-achievements/`

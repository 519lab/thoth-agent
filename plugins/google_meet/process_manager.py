...
session-scoped state file under ``$THOTH_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $THOTH_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_thoth_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this thoth install, leave it first —
    ...
    env = os.environ.copy()
    env["THOTH_MEET_URL"] = url
    env["THOTH_MEET_OUT_DIR"] = str(out)
    env["THOTH_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["THOTH_MEET_HEADED"] = "1"
    if auth_state:
        env["THOTH_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["THOTH_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if THOTH_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["THOTH_MEET_MODE"] = mode
    if realtime_model:
        env["THOTH_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["THOTH_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["THOTH_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["THOTH_MEET_REALTIME_KEY"] = realtime_api_key

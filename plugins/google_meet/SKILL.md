...
thoth meet install                 # pip deps + Chromium (transcribe only)
thoth meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
thoth meet auth                    # optional; skips guest-lobby wait
thoth meet setup                   # preflight checks
```

`thoth meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
pip install playwright websockets && python -m playwright install chromium
thoth plugins enable google_meet
thoth meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
thoth meet node approve my-mac ws://<mac-ip>:18789 <token>
thoth meet node ping my-mac                   # confirm reachable
```

Run `thoth meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Thoth agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `thoth meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `THOTH_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
Local:
```
$THOTH_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$THOTH_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `thoth meet node approve`.

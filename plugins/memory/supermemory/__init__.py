...
def _load_supermemory_config(thoth_home: str) -> dict:
    config = _default_config()
    config_path = Path(thoth_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, thoth_home: str) -> None:
    config_path = Path(thoth_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `thoth memory setup`.
        # All other options are documented for $THOTH_HOME/supermemory.json
    ...
    def save_config(self, values, thoth_home):
        ...
        _save_supermemory_config(sanitized, thoth_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from thoth_constants import get_thoth_home
        self._thoth_home = kwargs.get("thoth_home") or str(get_thoth_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._thoth_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "thoth", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "thoth_memory", "target": target, "type": "explicit_memory"},

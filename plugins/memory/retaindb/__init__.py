...
Config (env vars or thoth config.yaml under retaindb:):
...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Project resolution: RETAINDB_PROJECT > thoth-<profile> > "default"
        ...
        if explicit:
            ...
            project = f"thoth-{profile_name}" if (profile_name and profile_name not in {"", ".thoth"}) else "default"

        self._client = _Client(api_key, base_url, project)
        self._session_id = session_id
        self._user_id = kwargs.get("user_id", "default") or "default"
        self._agent_id = kwargs.get("agent_id", "thoth") or "thoth"

        from thoth_constants import get_thoth_home
        thoth_home_path = get_thoth_home()
        db_path = thoth_home_path / "retaindb_queue.db"
        self._queue = _WriteQueue(self._client, db_path)

        # Seed agent identity from SOUL.md in background
        soul_path = thoth_home_path / "SOUL.md"
        if soul_path.exists():

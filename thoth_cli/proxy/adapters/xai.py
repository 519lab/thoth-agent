...
# xAI's public API is OpenAI-compatible for the endpoints Thoth commonly
# uses. The Responses endpoint is included because Thoth's native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Thoth-managed OAuth credentials."""

    auth_hint = "thoth auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`thoth auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`thoth auth reset xai-oauth` or re-authenticate with "
                    "`thoth auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `thoth auth add xai-oauth --type oauth`."
            )

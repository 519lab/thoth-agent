...
- thoth config edit     - Open config in editor
- thoth config set      - Set a specific value
- thoth config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``thoth logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  thoth config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `thoth plugins enable
    # observability/langfuse`); credentials gate the plugin at runtime.
    "THOTH_LANGFUSE_ENV",
    "THOTH_LANGFUSE_RELEASE",
    "THOTH_LANGFUSE_SAMPLE_RATE",
    "THOTH_LANGFUSE_MAX_CHARS",
    "THOTH_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "pip":
        ...
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade thoth-agent"
        return "pip install --upgrade thoth-agent"
    return "thoth update"
...
def format_managed_message(action: str = "modify this Thoth installation") -> str:
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Thoth installation is managed by Homebrew "
            f"(THOTH_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade thoth-agent"
        )

    return (
        f"Cannot {action}: this Thoth installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Thoth."
    )
...
def _secure_dir(path):
    ...
    (e.g. THOTH_HOME_MODE=0701) for deployments where a web server (nginx,
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("THOTH_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Thoth runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("THOTH_CONTAINER") or os.environ.get("THOTH_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set THOTH_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Thoth-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Thoth auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Thoth sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Thoth
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Thoth sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Thoth should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``thoth chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: thoth sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Thoth truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``thoth kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``thoth kanban decompose`` and the kanban
        ...
        # ``thoth profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `thoth model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # e.g. `model · 68% · ~/projects/thoth`. Per-platform overrides go under
    ...
    # response. If not, Thoth feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Thoth auto-pauses the goal and
    ...
    # cron daemon.
    #
    # See `thoth curator status` for the last run summary.
    "curator": {
        "enabled": True,
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Thoth Agent*" header.
    ...
    # toggleable per-invocation via --accept-hooks or THOTH_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # Also overridable via THOTH_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `thoth -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `thoth` launches with
        ...
        # decomposition is manual via `thoth kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     thoth-agent's own python (sys.executable). Maximum isolation
    ...
    # update model picker lists without shipping a thoth-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://thoth.519lab.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `thoth model` invocation; network failures
    ...
    "sessions": {
        ...
        # default of ``thoth sessions prune``.
        "retention_days": 90,
    ...
    # `thoth tools`. These settings tune the backing Responses API call.
    "x_search": {
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'thoth model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Thoth auth store)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. THOTH_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #thoth)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Thoth API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Thoth API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Thoth API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # THOTH_TOOL_PROGRESS and THOTH_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.thoth.config`` entries, then checks
    ...
    try:
        ...
        # should never break `thoth update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Thoth versions) write ``models`` as
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'thoth setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Thoth won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'thoth doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("THOTH_TOOL_PROGRESS")
            old_mode = get_env_value("THOTH_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from THOTH_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from THOTH_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("THOTH_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from THOTH_TIMEZONE)")
            else:
                config["timezone"] = ""
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `thoth plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `thoth curator status` has no
    ...
    # metadata.thoth.config in their SKILL.md frontmatter.
...
_RENAMED_TOOLSET_SUFFIXES = (
    ...
    "gateway", "homeassistant", "matrix", "mattermost",
    "signal", "slack", "sms", "telegram", "webhook",
    "whatsapp", "acp", "cli",
)
_TOOLSET_RENAME_MAP = {
...
#   openai-codex (OAuth — thoth auth) — OpenAI Codex
#   nous         (OAuth — thoth auth) — Nous Portal
...
def load_env() -> Dict[str, str]:
    ...
    interactive menu render (`thoth tools`, `thoth setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `thoth tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Thoth env var names, avoiding false positives from values
...
def reload_env() -> int:
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Thoth vars that are no longer in .env
...
def show_config():
    ...
    print(color("│              ⚕ Thoth Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
...
# ``_inject_profile_env_vars`` surfaces them in ``thoth config`` UI so users

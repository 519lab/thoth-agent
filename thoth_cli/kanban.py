...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this THOTH_HOME and its
    ...
    Used by ``thoth kanban create`` (and callers) to warn when a task
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`thoth kanban daemon --force`)."
        )
    return (
        False,
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across Thoth profiles. "
    ...
    # THOTH_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `thoth kanban boards switch <slug>` or the "
            "THOTH_KANBAN_BOARD env var). Use `thoth kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_list.add_argument("--mine", action="store_true",
                        help="Filter by $THOTH_PROFILE as assignee")
    ...
    p_comment.add_argument("--author", default=None,
                           help="Author name (default: $THOTH_PROFILE or 'user')")
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `thoth gateway start`.",
    )
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.thoth/profiles/ and current assignees on the board)",
    )
    ...
    p_specify.add_argument(
        ...
        help="Author name recorded on the audit comment "
             "(default: $THOTH_PROFILE or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
    ...
    p_decompose.add_argument(
        ...
        default=None,
        help="Author name recorded on the audit comment "
             "(default: $THOTH_PROFILE or 'decomposer')",
    )
    p_decompose.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``thoth kanban …`` argparse dispatch.
    ...
    # ``board_exists`` which uses ``thoth_db.run_sync(connection())``;
    ...
    # subprocess-driven kanban CLI test (which doesn't have thoth_db
    # pre-initialised) crashes with ``RuntimeError: thoth_db.init()
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: thoth kanban <action> [options]\n"
                "Run 'thoth kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0
    ...
    # env-var pin for the duration of this call. Using THOTH_KANBAN_BOARD
    ...
    prev_board_env = os.environ.get("THOTH_KANBAN_BOARD")
    ...
    def _restore_board_env() -> None:
        ...
        if prev_board_env is None:
            ...
            os.environ["THOTH_KANBAN_BOARD"] = prev_board_env
    if board_override:
        try:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `thoth kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
        os.environ["THOTH_KANBAN_BOARD"] = normed
    ...
    # THOTH_HOME. Previously only `init` and `daemon` triggered
...
def _profile_author() -> str:
    ...
    for env in ("THOTH_PROFILE_NAME", "THOTH_PROFILE"):
...
# ---------------------------------------------------------------------------
# Boards management (thoth kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``thoth kanban boards <action>``.
    ...
    DB, so a fresh THOTH_HOME that has never called ``kanban init``
...
def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `thoth kanban boards delete <slug>` (alias), the
    ...
    if res["action"] == "archived":
        ...
        print(
            "Recover by running "
            f"`thoth kanban boards unarchive {res['slug']}` "
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    # the kanban dispatcher can use them without a separate `thoth profile
    ...
    if profiles:
        ...
        print("No profiles found under ~/.thoth/profiles/.")
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``thoth kanban show <id>``
...
def _worker_run_id_for(task_id: str) -> Optional[int]:
    if os.environ.get("THOTH_KANBAN_TASK") != task_id:
        return None
    raw = os.environ.get("THOTH_KANBAN_RUN_ID")
    if not raw:
        return None
    try:
        return int(raw)
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            "thoth kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    thoth gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`thoth kanban list --status ready` / "
                    f"`thoth kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real Thoth profile (i.e. one the

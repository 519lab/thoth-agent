...
Pure logic: walks a Thoth config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `thoth doctor` and a future `thoth migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Thoth config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Thoth config and return retirement issues.

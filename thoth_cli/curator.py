"""CLI subcommand: `thoth curator <subcommand>`.
    ...
    if dry:
        if synchronous:
            print(
                ...
                "`thoth curator status` and run `thoth curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`thoth curator unpin {args.skill}`"
        )
        return 1
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.thoth/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`thoth curator backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.thoth/skills/ tree (a safety "
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.thoth/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.thoth/skills/ from a curator snapshot "
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by thoth_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="thoth curator")
    register_cli(parser)

"""Shell completion script generation for thoth CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_thoth_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Thoth Agent bash completion
# Add to ~/.bashrc:
...
_thoth_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_thoth_profiles)" -- "$cur"))
        return
    fi
...
complete -F _thoth_completion thoth
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef thoth
# Thoth Agent zsh completion
# Add to ~/.zshrc:
...
_thoth() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_thoth_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'thoth command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Thoth Agent fish completion",
        "# Add to your config:",
        "#   thoth completion fish | source",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c thoth -f -s p -l profile"
        " -d 'Profile name' -xa '(__thoth_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__thoth_profiles)' -d 'Profile name'"
                )

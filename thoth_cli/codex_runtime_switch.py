...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Thoth's
...
The actual runtime resolution happens in thoth_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "thoth"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Thoth's MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Thoth for
        ...
        try:
            ...
            # Tools/MCP servers (excluding the thoth-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "thoth-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Thoth tool callback are always-on production
            ...
            if "thoth-tools" in mig_report.migrated:
                msg_lines.append(
                    "Thoth tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Thoth runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Thoth tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Thoth runtime.")

"""Migrate Thoth's MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Thoth's YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Thoth mcp_servers.<n>.command/args/env  → codex stdio transport
  Thoth mcp_servers.<n>.url/headers       → codex streamable_http transport
  Thoth mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Thoth mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Thoth-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Thoth's own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Thoth config.")
...
# Thoth keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Thoth's sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, thoth_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Thoth MCP server config to the codex inline-table dict
    ...
    if not isinstance(thoth_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(thoth_cfg.get("command"))
    has_url = bool(thoth_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (THOTH_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Thoth)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Thoth's managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Thoth's migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Thoth
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _build_thoth_tools_mcp_entry() -> dict:
    ...
    so a thoth installed under /opt/, /usr/local/, or a venv all work.
    THOTH_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # THOTH_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_thoth_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever THOTH_HOME its launcher sets
    ...
    # — that would override the launcher's THOTH_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("THOTH_HOME", tmp_path) would
    ...
    thoth_home = os.environ.get("THOTH_HOME") or ""
    if thoth_home and _looks_like_test_tempdir(thoth_home):
        thoth_home = ""
    if thoth_home:
        env["THOTH_HOME"] = thoth_home
    # PYTHONPATH passes through so a worktree-launched thoth finds the
    ...
    env["THOTH_QUIET"] = "1"
    env["THOTH_REDACT_SECRETS"] = env.get("THOTH_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    thoth_servers = (thoth_config or {}).get("mcp_servers") or {}
    if not isinstance(thoth_servers, dict):
        report.errors.append(
            "mcp_servers in Thoth config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in thoth_servers.items():
    ...
    # Inject Thoth's own tool surface as an MCP server so the spawned

"""
Thoth Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m thoth_cli.main web          # Start on http://127.0.0.1:9119
    python -m thoth_cli.main web --port 8080
"""
...
try:
    ...
    # running `thoth dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["THOTH_WEB_DIST"]) if "THOTH_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
_ACTION_LOG_DIR: Path = get_thoth_home() / "logs"
...
def _spawn_thoth_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    cmd = [sys.executable, "-m", "thoth_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "THOTH_NONINTERACTIVE": "1"},
    }
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Thoth supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in thoth_cli/config.py — listed here for deterministic ordering in the UI.
...
# the canonical ``thoth auth add <provider>`` command so the dashboard
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Thoth even
    when they also have a separate Thoth-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "thoth auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "thoth auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "thoth auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "thoth auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "thoth auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from thoth_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Thoth-managed PKCE file
    ...
    if provider_id in {"anthropic", "claude-code"}:
        ...
        try:
            from thoth_cli.auth import clear_provider_auth
...
# Guarded so thoth web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Thoth file AND credential pool.
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "thoth-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("THOTH_PORTAL_BASE_URL")
...
async def get_logs(
    ...
    from thoth_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_thoth_home() / "logs" / log_name
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, THOTH_HOME)."""
    from thoth_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process THOTH_HOME at import time. The dashboard is a single
...
async def get_skills():
    ...
    from thoth_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from thoth_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
#
# The endpoint spawns the same ``thoth --tui`` binary the CLI uses, behind
...
try:
    from thoth_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Thoth inside WSL2 to use the dashboard's /chat "
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/thoth"`` (no trailing slash) or ``""`` when
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/thoth/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /thoth`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__THOTH_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/thoth``)
        or empty string when served at root.
        """
        html = _index_path.read_text()
        chat_js = "true" if _DASHBOARD_EMBEDDED_CHAT_ENABLED else "false"
        token_script = (
            f'<script>window.__THOTH_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__THOTH_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__THOTH_BASE_PATH__="{prefix}";</script>'
        )
        if prefix:
    ...
    # under ``/thoth`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Thoth backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Thoth Teal",         "description": "Classic dark teal — the canonical Thoth look"},
    {"name": "default-large", "label": "Thoth Teal (Large)", "description": "Thoth Teal with bigger fonts and roomier spacing"},
...
def _discover_user_themes() -> list:
    ...
    """
    themes_dir = get_thoth_home() / "dashboard-themes"
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from thoth_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_thoth_home() / "plugins").resolve()
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"thoth auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from thoth_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from thoth_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from thoth_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from thoth_cli.plugins_cmd import dashboard_remove_user_plugin
...
def start_server(
    ...
    *,
    ...
    print(f"  Thoth Web UI → http://{host}:{port}")

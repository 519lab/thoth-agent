...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m thoth_cli.main gateway run --replace`` invocation) and
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Thoth profile."""
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("THOTH_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("THOTH_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("THOTH_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["THOTH_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["THOTH_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["THOTH_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("THOTH_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("THOTH_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("THOTH_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_script_path() -> Path:
    ...
    script_dir = Path(get_thoth_home()) / "gateway-service"
...
def _build_gateway_cmd_script(
    ...
    lines.append('set "THOTH_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports thoth_constants-based logic during startup.
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Thoth builds and failed
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m thoth_cli.main gateway run``
    ...
    log_dir = Path(get_thoth_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("THOTH_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("THOTH_GATEWAY_INSTALL_START_ON_LOGIN")
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `thoth -p <profile> gateway install` must be safe.
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"thoth {profile_arg} gateway start" if profile_arg else "thoth gateway start"
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Thoth gateway install prompt.")
                if start_now:
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from thoth_cli.config import get_thoth_home
        print(f"    type {Path(get_thoth_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_thoth_home()).resolve()}\\logs\\gateway-stdio.log")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Thoth gateway uninstall prompt.")
                    print(f"  Approve the Windows UAC prompt, then run: {cli_name()} gateway status")
                    return

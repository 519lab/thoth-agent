"""``thoth slack ...`` CLI subcommands.

Today only ``thoth slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ thoth slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of Thoth. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Thoth deployment — users can tweak them in the Slack UI after
    pasting.
    """
    from thoth_cli.commands import slack_app_manifest
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Thoth agent on Slack")[:140],
        ...
        "features": {
            ...
            "assistant_view": {
                "assistant_description": "Chat with Thoth in threads and DMs.",
            },
        },
        "oauth_config": {
            "scopes": {
...
def slack_manifest_command(args) -> int:
    ...
    """
    name = getattr(args, "name", None) or "Thoth"
    description = getattr(args, "description", None) or "Your Thoth agent on Slack"
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Thoth app\n"

"""
Single source of truth for provider identity in Thoth Agent.
...
2. **Thoth overlays** — transport type, auth patterns, aggregator flags,
...
# -- Thoth overlay ----------------------------------------------------------
# Thoth-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class ThothOverlay:
    """Thoth-specific provider metadata layered on top of models.dev."""
...
THOTH_OVERLAYS: Dict[str, ThothOverlay] = {
    "openrouter": ThothOverlay(
    ...
    "openai-codex": ThothOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "xai-oauth": ThothOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": ThothOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
    ...
    "copilot-acp": ThothOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": ThothOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": ThothOverlay(
    ...
    "kimi-for-coding": ThothOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": ThothOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": ThothOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": ThothOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": ThothOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": ThothOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": ThothOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": ThothOverlay(
        transport="openai_chat",
    ...
    "opencode": ThothOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": ThothOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "azure-foundry": ThothOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": ThothOverlay(
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Thoth overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Thoth overlay
    ...
    overlay = THOTH_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + thoth extra
    ...
    if overlay is not None:
        # Thoth-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in THOTH_OVERLAYS
    if provider == "bedrock":

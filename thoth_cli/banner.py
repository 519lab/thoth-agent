...
# Thoth — ibis-headed god of writing, bearing the was-staff and ankh (fits in left panel)
THOTH_IBIS = """[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣀⣀⡀[/]
[#FFD400]⠀⠀⠀⠀⠀⠀⠀⠀⣀⡴⠾⢿⣿⣿⣿⣦⣄[/]
[#FFD100]⠀⠀⠀⡀⠀⠀⢠⠞⠁⠀⣀⣼⣿⣿⣿⣿⣿⣦[/]
[#FFCE00]⠀⠀⠘⡆⠀⠀⢈⣤⣴⣾⣿⣿⣿⣿⣿⣿⣿⣿⡆[/]
...
def _fetch_pypi_latest(package: str = "thoth-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    ...
    embedded_rev = os.environ.get("THOTH_REVISION") or None
...
_RELEASE_URL_BASE = "https://github.com/519lab/thoth-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Thoth checkout. Cached per-process. Release URL always points at the
    canonical 519lab/thoth-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Thoth Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: Console, model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else THOTH_IBIS
    except Exception:
        _bskin = None
        _hero = THOTH_IBIS
    left_lines = ["", _hero, ""]
    ...
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]519lab[/]")

    if os.getenv("THOTH_YOLO_MODE"):
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else THOTH_AGENT_LOGO
        console.print(_logo)
        console.print()

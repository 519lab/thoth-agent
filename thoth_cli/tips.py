...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or thoth -c.",
    ...
    "Ctrl+Z suspends Thoth to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "thoth -c resumes your most recent CLI session. thoth -c \"project name\" resumes by title.",
    "thoth -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "thoth -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "thoth chat -t web,terminal enables only specific toolsets for a focused session.",
    "thoth chat -s github-pr-workflow preloads a skill at launch.",
    "thoth chat -q \"query\" runs a single non-interactive query and exits.",
    "thoth chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "thoth chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "thoth --yolo bypasses all dangerous command approval prompts for the entire session.",
    "thoth chat --source telegram tags the session for filtering in thoth sessions list.",
    "thoth -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "thoth doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "thoth dump outputs a compact setup summary — great for bug reports.",
    "thoth config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "thoth config edit opens config.yaml in your default editor.",
    "thoth config check scans for missing or stale configuration options.",
    "thoth sessions browse opens an interactive session picker with search.",
    "thoth sessions stats shows session counts by platform and database size.",
    "thoth sessions prune --older-than 30 cleans up old sessions.",
    "thoth skills search react --source skills-sh searches the skills.sh public directory.",
    "thoth skills check scans installed hub skills for upstream updates.",
    "thoth skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "thoth skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "thoth mcp add github --command npx adds MCP servers from the command line.",
    "thoth mcp serve runs Thoth itself as an MCP server for other agents.",
    "thoth auth add lets you add multiple API keys for credential pool rotation.",
    "thoth completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "thoth logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "thoth backup creates a zip backup of your entire Thoth home directory.",
    "thoth profile create coder creates an isolated profile that becomes its own command.",
    "thoth profile create work --clone copies your current config and keys to a new profile.",
    "thoth update syncs new bundled skills to ALL profiles automatically.",
    "thoth gateway install sets up Thoth as a system service (systemd/launchd).",
    "thoth memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "thoth webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: thoth tools disables unused tools, thoth skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "thoth models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as thoth -w).",
    ...
    "THOTH_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Thoth tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'thoth profile create coder' creates the 'coder' command.",
    "thoth profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Thoth prints a resume command with session ID and stats.",
    "thoth sessions export backup.jsonl exports all sessions for backup or analysis.",
    "thoth -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "thoth skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .thoth/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: thoth cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Thoth runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "thoth gateway install sets it up as a system service that starts on boot.",
    "BlueBubbles brings iMessage to Thoth via a local macOS server.",
    ...
    "Context files (.thoth.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.thoth/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Thoth loads project context from .thoth.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — thoth tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.thoth/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "thoth chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "thoth plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "thoth acp runs Thoth as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "THOTH_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "thoth login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "thoth status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.thoth/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Thoth your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.thoth/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Thoth auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.thoth/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at THOTH_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "THOTH_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in THOTH_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.thoth/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'thoth profile create ls' would be rejected.",
    "thoth profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "thoth claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "THOTH_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via thoth tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.thoth/.env.",
    "Skills from trusted repos (519lab) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.thoth/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Thoth auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Thoth config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Thoth is working.',
    ...
    '/reload reloads ~/.thoth/.env into the running session — pick up new API keys without restarting.',
    ...
    "THOTH_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.thoth/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.thoth/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'thoth curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "thoth curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'thoth curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'thoth auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'THOTH_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'THOTH_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'THOTH_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'THOTH_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.thoth/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.thoth/skills/ so drop-in skills appear without restarting the session.',
    ...
    'thoth -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'thoth chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'thoth chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'thoth chat --ignore-user-config skips ~/.thoth/config.yaml — reproducible bug reports and CI runs.',
    "thoth chat --source tool tags programmatic chats so they don't clutter thoth sessions list.",
    'thoth dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'thoth sessions rename <ID> "new title" renames any past session; thoth sessions delete <ID> removes one.',
    'thoth import restores a session export or profile archive produced by sessions export or profile export.',
    'thoth fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'thoth pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'thoth setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'thoth status --deep runs the full health sweep across every component; plain thoth status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'THOTH_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'THOTH_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.thoth/plugins/ — trust-gated by design.',
    "THOTH_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'THOTH_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'THOTH_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'THOTH_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'THOTH_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'THOTH_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'THOTH_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'THOTH_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'THOTH_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'THOTH_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'THOTH_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Thoth block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from thoth sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.thoth/dashboard-plugins/.',
]

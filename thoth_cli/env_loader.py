"""Helpers for loading Thoth .env files consistently across entrypoints."""
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `thoth setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from thoth_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_thoth_dotenv(
    *,
    ...
    # Resolve the home dir for the .env path. get_thoth_home() reads THOTH_HOME
    # and falls through to the disk default when unset, so no explicit
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of Thoth reads
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `thoth model`

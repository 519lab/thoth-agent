...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``THOTH_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``thoth kanban boards
...
# ---------------------------------------------------------------------------
# When THOTH_PG_DSN is set, kanban_db uses the shared asyncpg pool via
...
# ``THOTH_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``THOTH_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("THOTH_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def kanban_home() -> Path:
    """Return the shared Thoth root that anchors the kanban board.

    Resolution order:

    1. ``THOTH_KANBAN_HOME`` env var when set and non-empty (explicit
    ...
    """
    override = os.environ.get("THOTH_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``thoth kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``THOTH_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``thoth kanban boards
    ...
    """
    env = os.environ.get("THOTH_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``thoth kanban boards switch <typo>`` returns an error
...
def board_exists(board: Optional[str] = None) -> bool:
    ...
    a stale ``THOTH_KANBAN_BOARD`` env var pointing at an archived
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``THOTH_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("THOTH_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``THOTH_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("THOTH_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``thoth kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``THOTH_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    max_retries          INTEGER,
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``THOTH_SESSION_ID``. NULL
...
def _migrate_add_optional_columns(conn: "_PgConnection") -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``THOTH_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from thoth_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``thoth --skills ...`` alongside the built-in
    ...
    # `thoth --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``thoth kanban list --sort``.
...
def unlink_tasks(conn: _PgConnection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `thoth kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``thoth kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``thoth kanban complete <ready-task>
...
def _has_sticky_block(conn: _PgConnection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``thoth kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``thoth kanban complete <id>``) works without requiring
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Thoth
...
def _set_worker_pid(conn: _PgConnection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``thoth kanban
...
def has_spawnable_ready(conn: _PgConnection) -> bool:
    ...
    whose assignee maps to a real Thoth profile.
    ...
    if not rows:
        return False
    try:
        from thoth_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Thoth profile.
        # `_default_spawn` invokes ``thoth -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Thoth
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `thoth kanban tail` — without
...
def _kanban_worker_skill_available(thoth_home: Optional[str]) -> bool:
    ...
    the worker activates a profile (``thoth -p <name>``), its ``SKILLS_DIR``
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``thoth -p <profile> chat -q ...`` subprocess.
    ...
    ``THOTH_KANBAN_DB`` / ``THOTH_KANBAN_BOARD`` / workspaces_root env
    ...
    try:
        ...
        # _apply_profile_override() via THOTH_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # THOTH_HOME never had profiles created.
        pass
    if task.tenant:
        env["THOTH_TENANT"] = task.tenant
    env["THOTH_KANBAN_TASK"] = task.id
    env["THOTH_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["THOTH_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["THOTH_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["THOTH_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    env["THOTH_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["THOTH_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["THOTH_KANBAN_BOARD"] = resolved_board
    # THOTH_PROFILE is the author the kanban_comment tool defaults to.
    # `thoth -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["THOTH_PROFILE"] = profile_arg
    ...
    # `thoth kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`thoth` executable not found on PATH. "
            "Install Thoth Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``thoth kanban daemon`` is systemd-friendly.
...
def known_assignees(conn: _PgConnection) -> list[dict]:
    ...
    - ``thoth kanban assignees`` for the terminal.

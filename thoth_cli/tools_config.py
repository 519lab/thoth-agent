"""
Unified tool configuration for Thoth Agent.

`thoth tools` and `thoth setup tools` both enter this module.
...
from thoth_cli.nous_subscription import (
...
from thoth_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _THOTH_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `thoth tools` → Video Generation, which walks
...
# `thoth tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from thoth_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `thoth tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``thoth tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from thoth_cli.plugins import discover_plugins, get_plugin_toolsets
...
from thoth_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Thoth routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. THOTH_CUA_DRIVER_VERSION is an
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("THOTH_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``thoth update`` and
      by ``thoth computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `thoth update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Thoth process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_thoth_home()}/app && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in app directory)")
            return
        ...
        if not camofox_dir.exists() and _npm_bin:
            ...
            # package.json so that `thoth update` does not silently pull
        ...
        try:
            from thoth_cli.auth import login_spotify_command
        except Exception as exc:
        ...
        try:
            ...
            # toolset enable; they can retry with `thoth auth spotify`.
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `thoth auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: thoth auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: thoth auth add xai-oauth"
                )
        else:
...
def _get_platform_tools(
    ...
    *,
    ...
    # "thoth-cli" (which include all _THOTH_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[thoth-cli, spotify]`` after enabling Spotify via ``thoth
        ...
        # (e.g. "thoth-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``thoth tools`` to flip the toolset on. Only fires when
    ...
    # Recover non-configurable platform toolsets (e.g. discord admin
    # sub-toolsets).  These are part of the platform's default composite but
    # absent from CONFIGURABLE_TOOLSETS, so they can't appear in the TUI
    # checklist or in a user-saved config.  Must run in BOTH branches —
    # otherwise saving via `thoth tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"thoth-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("thoth-")}
    ...
    # shipped as a bundled plugin but user must opt in via `thoth tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `thoth tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by thoth tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (thoth-cli, thoth-telegram, etc.)
    ...
    # Opening `thoth tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `thoth tools` gets a silent no-op
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `thoth tools` and `thoth setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Thoth Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://thoth.519lab.com/docs/user-guide/features/tools", Colors.DIM))
    print()

"""Profile distributions — shareable, packaged Thoth profiles via git.

A distribution is a Thoth profile published as a git repository (or
...
* ``thoth profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``thoth skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``thoth profile``, not a parallel tree):

    thoth profile install <source> [--name N] [--alias] [--force] [--yes]
...
def check_thoth_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Thoth {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``thoth profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Thoth profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Thoth distribution."
        )
    ...
    with tempfile.TemporaryDirectory(prefix="thoth_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `thoth profile update` to upgrade in place, "

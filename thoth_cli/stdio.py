...
on Japanese locales, etc.).  Thoth's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``thoth_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Thoth is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``THOTH_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("THOTH_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Thoth-managed Git install directories so
    ...
    # already-running shells don't see the change — which means thoth
    ...
    # install without Thoth-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Thoth's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Thoth's
    ``thoth config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Thoth (User env var in Windows Settings, or
    export in a PowerShell profile) and Thoth picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Thoth-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\thoth\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``thoth`` from right after install) retain their old PATH.

    Any subprocess Thoth spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Thoth-managed tool directories to our

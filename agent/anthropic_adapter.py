"""Anthropic Messages API adapter for Thoth Agent.

Translates between Thoth's internal OpenAI-style message format and
...
# Thoth effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Thoth historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Thoth)
    ...
    # 1. Thoth-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Thoth configs.
...
def run_thoth_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    print()
    print("Authorize Thoth with your Claude Pro/Max subscription.")
    print()
...
def read_thoth_oauth_credentials() -> Optional[Dict[str, Any]]:
    ...
    if _THOTH_OAUTH_FILE.exists():
        try:
            ...
            logger.debug("Failed to read Thoth OAuth credentials: %s", e)
    return None
...
def build_anthropic_kwargs(
    ...
    # silently hides reasoning text that Thoth surfaces in its CLI. We

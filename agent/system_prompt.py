...
upstream prefix cache warm.  See ``thoth-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Thoth never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want THOTH_HOME persona while keeping
    ...
    # THOTH_KANBAN_TASK env var). Normal chat sessions never see
    ...
    if not agent.skip_context_files:
        ...
        # mode).  The gateway process runs from the thoth-agent install
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Thoth never rebuilds or reinjects parts of it

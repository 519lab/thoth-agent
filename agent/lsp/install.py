...
appropriate.  All installs go to a Thoth-owned bin staging dir,
``<THOTH_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``thoth lsp
  status``.
...
# ``<THOTH_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = thoth_lsp_bin_dir().parent  # <THOTH_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a thoth-owned target dir.
    ...
    console script.
    """
    pip_target = thoth_lsp_bin_dir().parent / "python-packages"
    ...
    if bin_path.exists():
        link = thoth_lsp_bin_dir() / bin_name
        if not link.exists():
            try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``thoth lsp status`` CLI to give users a quick

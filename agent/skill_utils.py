...
from thoth_constants import get_config_path, get_skills_dir, is_termux
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``THOTH_PLATFORM`` or
            ``THOTH_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``thoth`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.thoth/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``thoth`` cold-start time
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.thoth/skills/`` first, then external.
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Thoth metadata, VCS, virtualenv/dependency, and cache

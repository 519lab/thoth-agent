...
from thoth_logging import set_session_context
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Thoth tool use: "
    ...
    return (
        ...
        f"context, but Thoth needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Thoth config, set `model.ollama_num_ctx: 65536` "
...
def run_conversation(
    ...
    # ``thoth logs --session <id>`` can filter a single conversation.
    from thoth_logging import set_session_context
    ...
    # are reused.  The system prompt is Thoth's territory; plugins
    ...
    # all run inside Codex). Default Thoth path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Thoth internals.
        #
        # Thoth invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Thoth tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if env_var_enabled("THOTH_DUMP_REQUESTS"):
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: thoth login --provider nous")
                ...
                if (
                    ...
                    from thoth_constants import display_thoth_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Thoth-managed OAuth/setup tokens")
                ...
                # which Thoth's system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Thoth's system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`thoth",
                        force=True,
                    )
                    agent._vprint(
    ...
    if final_response is None and (
        ...
        _kanban_task = os.environ.get("THOTH_KANBAN_TASK")
        if _kanban_task:
            try:

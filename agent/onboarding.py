...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Thoth starts and finds ``~/.openclaw/``.

    Points users at ``thoth claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``thoth claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Thoth, run "
        "`thoth claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `thoth claw cleanup` (renames it to ~/.openclaw.pre-migration — "

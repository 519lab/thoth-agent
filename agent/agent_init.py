...
from thoth_cli.timeouts import get_provider_request_timeout
...
def init_agent(
    ...
    agent._cache_ttl = "5m"
    try:
        from thoth_cli.config import load_config as _load_pc_cfg
    ...
    if agent.verbose_logging:
        ...
        # on the fact that thoth_logging.setup_logging() does not
        ...
        # handler level inside thoth_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from thoth_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `thoth model` to "
                        "select a provider, or run `thoth setup` for first-time "
                        "configuration."
                    )
    ...
    try:
        from thoth_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from thoth_cli.config import load_config as _load_agent_config
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from thoth_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from thoth_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Thoth Agent.  Choose a model with at least "

...
Storage
-------
Bundles live in ``~/.thoth/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``THOTH_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``thoth bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under THOTH_HOME.

    Honors ``THOTH_BUNDLES_DIR`` for tests; falls back to
    ``<THOTH_HOME>/skill-bundles``.
    """
    override = os.environ.get("THOTH_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_thoth_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `thoth bundles` CLI subcommand.
# ---------------------------------------------------------------------------

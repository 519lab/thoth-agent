...
Plugins built on Thoth Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Thoth
...
:class:`~thoth_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Thoth's main config
...
handles every provider, fallback chain, and per-task override Thoth
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`thoth_cli.plugins.PluginContext`

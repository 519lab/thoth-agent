...
projection into Thoth's display, approval bridging, transcript projection into
...
"codex_app_server"`. Thoth's default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        if spawn_env.get("THOTH_KANBAN_TASK"):
            kanban_db = spawn_env.get("THOTH_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "THOTH_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("THOTH_HOME", os.path.expanduser("~/.thoth")),
                        "kanban",
                    ),
                )
            )

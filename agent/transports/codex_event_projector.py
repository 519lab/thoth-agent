"""Projects codex app-server events into Thoth's messages list.

The translator that lets Thoth's memory/skill review keep working under the
...
Thoth's message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Thoth
...
def _deterministic_call_id(item_type: str, item_id: str) -> str:
    ...
    prefix caches stay valid. See CLAUDE.md (deterministic tool-call IDs keep
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Thoth's existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Thoth stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Thoth already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Thoth's messages store text only.
        text_parts: list[str] = []

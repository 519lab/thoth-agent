"""Lightweight internationalization (i18n) for Thoth static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by Thoth itself -- approval prompts, a handful of gateway slash
...
Usage::
    ...
    print(t("approval.choose_long", lang="ja"))            # explicit override

Language resolution order:
    1. Explicit ``lang=`` argument passed to :func:`t`
    2. ``THOTH_LANGUAGE`` environment variable (for tests / quick override)
    3. ``display.language`` from config.yaml
    4. ``"en"`` (baseline)

Supported languages: en, ja, de, es, fr, tr, uk.  Unknown values fall back to en.
"""
...
# Accept a few natural aliases so users who type "japanese" / "jp" / "de-DE"
...
def _normalize_lang(value: Any) -> str:
    ...
    Accepts supported codes directly, common aliases (``japanese`` -> ``ja``),
    and case-insensitive regional tags (``ja-JP`` -> ``ja``).  Returns the
    ...
    # Try stripping a region suffix (e.g. "de-CH" -> "de", "ja-JP" -> "ja").
    base = key.split("-", 1)[0]
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a thoth dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`thoth_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("THOTH_LANGUAGE")
    if env_lang:

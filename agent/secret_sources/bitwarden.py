...
Thoth pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.thoth/.env``.
...
* The access token is stored in ``~/.thoth/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``thoth`` invocations don't
  hammer the API.
* Failures NEVER block Thoth startup.  Missing binary, no network,
...
def install_bws(*, force: bool = False) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="thoth-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "thoth-agent"})
    try:
    ...
    *,
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `thoth secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `thoth secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `thoth secrets bitwarden setup` to install."
        )
        return result

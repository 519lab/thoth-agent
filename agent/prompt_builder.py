...
from thoth_constants import get_thoth_home, get_skills_dir, is_wsl
...
_THOTH_MD_NAMES = (".thoth.md", "THOTH.md")


def _find_thoth_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.thoth.md`` or ``THOTH.md``.
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.thoth/kanban.db`. Your task id is in "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `thoth kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Thoth WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Thoth itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Thoth runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Thoth itself is running. The host OS, home, and cwd "
                f"of the Thoth process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Thoth "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_thoth_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.thoth/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Thoth Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `thoth-agent` skill "
            "first. It has the actual commands (e.g. `thoth config set …`, `thoth tools`, "
            "`thoth setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: thoth setup, thoth setup tools, thoth setup terminal, thoth status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from THOTH_HOME and return its content, or None.
    ...
    try:
        ...
        logger.debug("Could not ensure THOTH_HOME before loading SOUL.md: %s", e)
...
def _load_thoth_md(cwd_path: Path) -> str:
    """.thoth.md / THOTH.md — walk to git root."""
    ...
    try:
        ...
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".thoth.md")
    except Exception as e:
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .thoth.md / THOTH.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from THOTH_HOME is independent and always included when present.
    ...
    # SOUL.md from THOTH_HOME only — skip when already loaded as identity
    if not skip_soul:

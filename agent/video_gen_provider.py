...
as ``kind: backend``) or ``~/.thoth/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``thoth tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``thoth tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``thoth tools`` picker."""
        return {
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``thoth tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$THOTH_HOME/cache/videos/``, creating parents as needed."""
    from thoth_constants import get_thoth_home

    path = get_thoth_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$THOTH_HOME/cache/videos/``.

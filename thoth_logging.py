"""Centralized logging setup for Thoth Agent.
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_thoth_session_injector", False):
    ...
    _session_record_factory._thoth_session_injector = True  # type: ignore[attr-defined]
...
def setup_logging(
    *,
    ...
    """Configure the Thoth logging subsystem.
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_thoth_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._thoth_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the thoth group. However, both _open() (initial creation) and

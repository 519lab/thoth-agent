# OpenAI-Compatible API Server for Thoth Agent
...
SSE streaming. By exposing this endpoint, thoth-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  thoth-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.thoth/.env`
2. Run `thoth gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with thoth-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns thoth-agent as a model) |
...
Authorization: Bearer thoth-api-key-here
...
But thoth-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "thoth-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured thoth-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of thoth-agent's core prompt
...
Once implemented, thoth-agent works as a drop-in backend for:

...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("thoth/thoth-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned thoth/* and pr-* branches."""

    def test_prunes_orphaned_thoth_branch(self, git_repo):
        """thoth/thoth-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "thoth/thoth-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "thoth/thoth-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "thoth/thoth-deadbeef" in result.stdout
        ...
        assert "thoth/thoth-deadbeef" in orphaned
        ...
        assert "thoth/thoth-deadbeef" not in result.stdout

"""
Tests for timezone support (thoth_time module + integration points).
...
def _reset_thoth_time_cache():
    """Reset the thoth_time module cache (replacement for removed reset_cache)."""
    thoth_time._cached_tz = None
    thoth_time._cached_tz_name = None
    thoth_time._cache_resolved = False


# =========================================================================
# thoth_time.now() — core helper
# =========================================================================

class TestThothTimeNow:
    ...
    def teardown_method(self):
        _reset_thoth_time_cache()
        os.environ.pop("THOTH_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["THOTH_TIMEZONE"] = "Asia/Kolkata"
        result = thoth_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["THOTH_TIMEZONE"] = "UTC"
        result = thoth_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["THOTH_TIMEZONE"] = "America/New_York"
        result = thoth_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["THOTH_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="thoth_time"):
            result = thoth_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["THOTH_TIMEZONE"] = "Europe/London"
        tz = thoth_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["THOTH_TIMEZONE"] = "Not/A/Timezone"
        tz = thoth_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When THOTH_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) THOTH_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("THOTH_TIMEZONE=" + os.environ.get("THOTH_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "THOTH_TIMEZONE=NOT_SET" in result["output"], (
            "THOTH_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When THOTH_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("THOTH_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_normalizes_aware_to_thoth_tz(self):
        """Already-aware datetimes should be normalized to Thoth tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Thoth tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Thoth tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Thoth is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Thoth tz
        ...
        os.environ["THOTH_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_thoth_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Thoth timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["THOTH_TIMEZONE"] = "US/Eastern"
        _reset_thoth_time_cache()

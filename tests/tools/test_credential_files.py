...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(thoth_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.thoth/token.json"
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.thoth/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        ...
        with patch.dict(os.environ, {"THOTH_HOME": str(thoth_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.thoth")

        assert mounts[0]["container_path"] == "/home/user/.thoth/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.thoth/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.thoth/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside thoth_home
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a sensitive file outside thoth_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside thoth_home pointing outside
        symlink = thoth_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes THOTH_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, thoth_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(thoth_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(thoth_home, [str(sensitive)])
    ...
    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        ...
        (thoth_home / "oauth.json").write_text("{}")
        self._write_config(thoth_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        assert "/root/.thoth/cache/documents" in paths
        assert "/root/.thoth/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.thoth/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        (thoth_home / "image_cache").mkdir()
        monkeypatch.setenv("THOTH_HOME", str(thoth_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(thoth_home / "document_cache") in host_paths
        assert str(thoth_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.thoth/cache/documents" in container_paths
        assert "/root/.thoth/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        assert entries[0]["container_path"] == "/root/.thoth/cache/screenshots/session_abc/screen1.png"

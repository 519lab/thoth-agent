...
def _clean_state():
    ...
    saved = {}
    for k in ("THOTH_INTERACTIVE", "THOTH_GATEWAY_SESSION", "THOTH_EXEC_ASK", "THOTH_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["THOTH_INTERACTIVE"] = "1"
        session_key = os.getenv("THOTH_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No THOTH_INTERACTIVE or THOTH_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True

...
class TestXAIProviderIsAvailable:
    ...
    auth-store lock. It runs on every ``thoth tools`` repaint and at
...
class TestXAIBackendWiring:
    ...
    def test_is_backend_available_does_not_call_resolver(self, monkeypatch):
        ...
        dispatch and every `thoth tools` repaint. It must not invoke the
    ...
    def test_xai_not_in_legacy_backend_candidate_chain(self, monkeypatch):
        ...
        chain is for users who set env vars but never ran ``thoth tools``,
...
class TestXAIProviderOAuthPath:
    ...
    runtime resolver (``thoth auth`` sign-in) rather than an env-var key.
    Patches at the ``thoth_cli.runtime_provider.resolve_runtime_provider``

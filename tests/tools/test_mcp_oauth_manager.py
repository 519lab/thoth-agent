...
def test_thoth_provider_subclass_exists():
    """ThothMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _THOTH_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _THOTH_PROVIDER_CLS is not None
    assert issubclass(_THOTH_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_thoth_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns ThothMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _THOTH_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    ...
    assert _THOTH_PROVIDER_CLS is not None
    assert isinstance(provider, _THOTH_PROVIDER_CLS)
    assert provider._thoth_server_name == "srv"

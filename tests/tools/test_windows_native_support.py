...
code paths Thoth takes on native Windows.
...
class TestConfigureWindowsStdio:
    ...
    - respect THOTH_DISABLE_WINDOWS_UTF8 opt-out
    """
    ...
    def test_windows_path_sets_env_and_reconfigures_streams(self, monkeypatch):
        ...
        monkeypatch.delenv("THOTH_DISABLE_WINDOWS_UTF8", raising=False)
    ...
    def test_disable_flag_short_circuits(self, monkeypatch, optout):
        ...
        monkeypatch.setattr(stdio, "is_windows", lambda: True)
        monkeypatch.setenv("THOTH_DISABLE_WINDOWS_UTF8", optout)
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("thoth_cli.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "thoth_cli" / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    ...
    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "thoth_cli/main.py", "gateway/run.py"],
    )
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "thoth_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestCodeExecutionTransportTcpFallback:
    ...
    AF_INET endpoints based on the THOTH_RPC_SOCKET format.
    """
...
class TestLocalEnvironmentWindowsTempDir:
    ...
    def test_source_has_windows_branch_using_thoth_home(self):
        root = Path(__file__).resolve().parents[2]
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_thoth_cli_gateway_uses_compat_kwargs(self):
        ...
        assert "windows_detach_popen_kwargs" in source, (
            "thoth_cli/gateway.py must use the platform-aware detach helper"
        )

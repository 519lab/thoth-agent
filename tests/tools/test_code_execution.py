...
class TestThothToolsGeneration(unittest.TestCase):
    ...
    def test_generates_subset(self):
        src = generate_thoth_tools_module(["terminal", "web_search"])
    ...
    def test_empty_list_generates_nothing(self):
        src = generate_thoth_tools_module([])
    ...
    def test_non_allowed_tools_ignored(self):
        src = generate_thoth_tools_module(["vision_analyze", "terminal"])
        self.assertIn("def terminal(", src)
        self.assertNotIn("def vision_analyze(", src)

    def test_rpc_infrastructure_present(self):
        src = generate_thoth_tools_module(["terminal"])
        self.assertIn("THOTH_RPC_SOCKET", src)
    ...
    def test_uds_transport_serializes_concurrent_calls(self):
        ...
        src = generate_thoth_tools_module(["terminal"], transport="uds")
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        self.assertIn("THOTH_RPC_DIR=/data/data/com.termux/files/usr/tmp/thoth_exec_", run_cmd)
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import thoth_constants; print(thoth_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("thoth_constants.py", result["output"])
...
class TestBuildExecuteCodeSchema(unittest.TestCase):
    ...
    def test_real_scenario_all_sandbox_tools_disabled(self):
        ...
        Scenario: user runs `thoth tools code_execution` (only code_execution
    ...
    def test_real_scenario_only_vision_enabled(self):
        """Another real path: user runs `thoth tools code_execution,vision`.
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_thoth_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("THOTH_RPC_SOCKET", child_env)
...
class TestLoadConfig(unittest.TestCase):
    ...
    def test_returns_code_execution_section(self):
        from tools.code_execution_tool import _load_config
        with patch("thoth_cli.config.read_raw_config",
    ...
    def test_does_not_import_interactive_cli(self):
        ...
        with patch.dict("sys.modules", {"cli": mock_cli}), \
             patch("thoth_cli.config.read_raw_config", return_value={}):
            result = _load_config()

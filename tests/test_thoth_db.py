...
def test_ensure_pool_sync_returns_false_when_no_dsn(monkeypatch):
    ...
    thoth_db._pool = None
    try:
        assert thoth_db.ensure_pool_sync() is False
        assert thoth_db._pool is None
    finally:
        thoth_db._pool = saved_pool
...
def test_run_sync_executes_coroutine_synchronously():
    async def make_seven():
        return 7
    assert thoth_db.run_sync(make_seven()) == 7
...
def test_ssl_kwarg_skips_ssl_for_local_dsns():
    ...
    for dsn in (
        "postgresql://thoth:thoth@localhost:5432/thoth",
        "postgresql://thoth:thoth@127.0.0.1:5433/thoth",
        "postgresql://thoth:thoth@[::1]:5432/thoth",
        "postgresql://thoth:thoth@postgres:5432/thoth",
        "postgresql://thoth:thoth@postgres-test:5432/thoth",
        "postgresql://thoth:thoth@db:5432/myapp",
    ):
        assert thoth_db._ssl_kwarg_for_dsn(dsn) == {"ssl": False}, dsn
...
def test_ssl_kwarg_respects_explicit_sslmode_in_dsn():
    ...
    connection."""
    dsn = "postgresql://thoth:thoth@localhost:5432/thoth?sslmode=require"
    assert thoth_db._ssl_kwarg_for_dsn(dsn) == {}
...
def test_ssl_kwarg_lets_asyncpg_negotiate_remote_dsns():
    ...
    for dsn in (
        ...
        assert thoth_db._ssl_kwarg_for_dsn(dsn) == {}, dsn
...
def test_run_on_pool_loop_routes_across_loops(thoth_db_dsn):
    ...
    try:
        assert thoth_db._pool._loop is thoth_db._get_sync_loop()

        async def on_other_loop():
            running = asyncio.get_running_loop()
            # Prove we're genuinely cross-loop before routing.
            assert thoth_db._pool._loop is not running

            async def _q():
                async with thoth_db.connection() as conn:
                    return await conn.fetchval("SELECT 7")

            return await thoth_db.run_on_pool_loop(_q())
...
def test_db_loop_runs_continuously_on_its_own_thread():
    ...
    loop = thoth_db._get_sync_loop()
    assert loop.is_running()
    assert thoth_db._db_thread is not None
    assert thoth_db._db_thread.is_alive()
    assert thoth_db._db_thread.daemon is True
...
def test_run_sync_accepts_non_coroutine_awaitables(thoth_db_dsn):
    ...
    assert thoth_db.ensure_pool_sync() is True
    try:
        conn = thoth_db.run_sync(thoth_db.pool().acquire())
        try:
            assert thoth_db.run_sync(conn.fetchval("SELECT 3")) == 3
        finally:
            thoth_db.run_sync(thoth_db.pool().release(conn))
    finally:
        thoth_db.run_sync(thoth_db.close())
...
def test_long_lived_task_survives_after_routed_boot_returns(thoth_db_dsn):
    ...
    try:
        ...
        assert thoth_db.run_sync(_boot_that_spawns_long_lived_task()) == "booted"
...
class TestPoolMaxInactiveLifetime:
    ...
    def _reset_pool(self):
        ...
        saved = thoth_db._pool
        thoth_db._pool = None
        yield
        thoth_db._pool = saved if saved is not None else None
        if saved is None:
            thoth_db._pool = None
    ...
    async def _capture_create_pool_kwargs(self):
        ...
        thoth_db._pool = None  # drop the mock pool

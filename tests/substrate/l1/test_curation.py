...
@pytest.mark.asyncio
async def test_merge_repoints_relationships_and_citations(thoth_db_initialized):
...
@pytest.mark.asyncio
async def test_merge_dedups_duplicate_relationships(thoth_db_initialized):
...
@pytest.mark.asyncio
async def test_merge_unknown_or_identical_returns_false(thoth_db_initialized):
    from uuid import uuid4
...
@pytest.mark.asyncio
async def test_forget_entity_cascades(thoth_db_initialized):
    import thoth_db
...
@pytest.mark.asyncio
async def test_edit_entity_summary_and_name(thoth_db_initialized):
...
@pytest.mark.asyncio
async def test_duplicate_candidates_suggests_similar(thoth_db_initialized):
...
@pytest.mark.asyncio
async def test_cli_merge_and_forget_behaviour(thoth_db_initialized):
    import thoth_db

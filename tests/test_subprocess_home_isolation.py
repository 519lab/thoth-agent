...
See: https://github.com/519lab/thoth-agent/issues/4426
"""
...
class TestGetSubprocessHome:
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        from thoth_constants import (
            get_thoth_home,
            reset_thoth_home_override,
            set_thoth_home_override,
        )
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_thoth_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_thoth_home_override(profile)
        try:
            assert get_thoth_home() == profile
            release.set()
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from thoth_cli.profiles import _PROFILE_DIRS

...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    thoth_state = ModuleType("thoth_state")
    thoth_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "thoth_state", thoth_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Thoth"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Thoth"

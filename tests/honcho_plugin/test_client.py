...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "thoth"
        assert config.workspace_id == "thoth"
...
class TestFromGlobalConfig:
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"thoth": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"thoth": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"thoth": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveActiveHost:
    def test_default_returns_thoth(self):
        with patch.dict(os.environ, {}, clear=True):
            ...
            os.environ.pop("THOTH_HOME", None)
            assert resolve_active_host() == "thoth"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"THOTH_HONCHO_HOST": "thoth.coder"}):
            assert resolve_active_host() == "thoth.coder"
    ...
    def test_default_profile_returns_thoth(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("THOTH_HONCHO_HOST", None)
            with patch("thoth_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "thoth"

    def test_custom_profile_returns_thoth(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("THOTH_HONCHO_HOST", None)
        ...
        with patch.dict(os.environ, {}, clear=False):
            ...
            try:
                ...
                if saved is not None:
                    sys.modules["thoth_cli.profiles"] = saved
                else:
                    sys.modules.pop("thoth_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="thoth.coder")
        assert config.host == "thoth.coder"
        assert config.workspace_id == "thoth"  # shared workspace
        assert config.ai_peer == "thoth.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="thoth")
        assert config.host == "thoth"
        assert config.workspace_id == "thoth"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "thoth": {"aiPeer": "thoth", "peerName": "alice"},
                "thoth.coder": {
                    "aiPeer": "thoth.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="thoth.coder", config_path=config_file,
        )
        assert config.host == "thoth.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "thoth.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="thoth.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "thoth.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"thoth": {"enabled": True, "aiPeer": "thoth"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"thoth": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_thoth_config_timeout_override_used_when_config_timeout_missing(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("thoth_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("thoth_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_thoth_request_timeout_alias_used(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("thoth_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)

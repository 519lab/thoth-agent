...
def _run_gateway_import(thoth_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
...
@pytest.fixture
def thoth_home(tmp_path: Path) -> Path:
...
def test_config_gateway_timeout_wins_over_stale_env(thoth_home: Path) -> None:
    """Every agent.* bridge key must be config-authoritative, not .env-authoritative."""
    _write_config(thoth_home, agent_cfg={

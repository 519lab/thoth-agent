...
Supergroup forum topics route with ``message_thread_id``. Thoth-created
...
async def test_send_typing_attempts_api_call_for_dm_topic_reply_fallback():
    """Thoth-created DM topic lanes should still attempt scoped typing.
...
async def test_send_uses_metadata_reply_fallback_for_streaming_dm_topics():
    """Metadata-only sends still stay in Thoth-created Telegram DM topics."""
    adapter = _make_adapter()
    call_log = []

...
"""
    ...
    assert "Tyler Williams > Active Projects: Thoth Agent" in entries
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # thoth_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.thoth/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "thoth_config_mod"}
...
def test_rebrand_text_preserves_filesystem_path_casing():
    ...
    rewritten to ``~/.Thoth/config.yaml`` — a path that doesn't exist —

"""Tests for thoth backup and import commands."""
...
def _make_thoth_tree(root: Path) -> None:
    ...
    (root / "thoth_state.db").write_bytes(b"fake-state")
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Thoth backup."""
        from thoth_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only thoth_state.db (old wrong name) is rejected."""
        from thoth_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["thoth_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("THOTH_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from thoth_cli.backup import run_backup, run_import
...
class TestFormatSize:
    def test_bytes(self):
        from thoth_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without thoth markers fails validation."""
        import io
        from thoth_cli.backup import _validate_backup_zip
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert "thoth -p coder" in coder_wrapper
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        import thoth_cli.backup as backup_mod
        original_import = __builtins__.__import__ if hasattr(__builtins__, '__import__') else __import__

        def fake_import(name, *a, **kw):
            if name == "thoth_cli.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from thoth_cli.backup import _safe_copy_db
...
# ---------------------------------------------------------------------------
# Pre-update backup (thoth update safety net)
# ---------------------------------------------------------------------------

class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``thoth update``
...
class TestRunPreUpdateBackup:
    ...
    def thoth_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for thoth_cli.config + thoth_constants so they pick up THOTH_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("thoth_cli.config") or mod == "thoth_constants":
    ...
    def test_config_enabled_creates_backup(self, thoth_home, capsys):
        ...
        for mod in list(_sys.modules.keys()):
            if mod.startswith("thoth_cli.config"):
...
# ---------------------------------------------------------------------------
# Pre-migration backup (thoth claw migrate safety net)
# ---------------------------------------------------------------------------

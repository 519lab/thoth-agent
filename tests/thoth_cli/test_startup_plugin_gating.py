...
``thoth_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``thoth --help``, ``thoth version``,
``thoth logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``thoth --help`` in-process and parse the subcommand block.
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``thoth help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"thoth_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "

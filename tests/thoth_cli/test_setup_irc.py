"""Tests for IRC gateway configuration via `thoth setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Thoth install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Thoth install."""
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `thoth setup gateway` experience with IRC present."""

"""Tests for the update check mechanism in thoth_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("thoth_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("THOTH_HOME", str(tmp_path))
    with patch("thoth_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    import thoth_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "thoth_cli" / "banner.py"
    ...
    monkeypatch.setenv("THOTH_HOME", str(tmp_path))
    with patch("thoth_cli.banner.subprocess.run") as mock_run:
        with patch("thoth_cli.banner.check_via_pypi", return_value=0):
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when THOTH_HOME has no git repo."""
    import thoth_cli.banner as banner
    ...
    # Point THOTH_HOME at a temp dir with no hermes-agent/.git
    monkeypatch.setenv("THOTH_HOME", str(tmp_path))
    with patch("thoth_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from thoth_cli.main import _invalidate_update_cache

    # Build a fake ~/.thoth with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"THOTH_HOME": str(default_home)}):

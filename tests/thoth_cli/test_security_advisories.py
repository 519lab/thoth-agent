"""Tests for thoth_cli.security_advisories.
...
import thoth_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect THOTH_HOME so banner cache and config writes are sandboxed."""
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full thoth_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "thoth_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "thoth_cli.config.save_config",

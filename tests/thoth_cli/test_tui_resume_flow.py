...
def test_termux_bundled_skills_stamp_controls_sync(monkeypatch, tmp_path, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(main_mod, "get_thoth_home", lambda: tmp_path)
    ...
    monkeypatch.setenv("THOTH_TERMUX_FORCE_SKILLS_SYNC", "1")
    assert main_mod._termux_bundled_skills_sync_needed() is True
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from thoth_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "thoth -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """thoth -z bypasses ThothCLI, but recall still needs SessionDB."""
    from thoth_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(
        sys.modules,
        "thoth_cli.config",
        mod("thoth_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "thoth_cli.models",
        mod("thoth_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "thoth_cli.runtime_provider",
        mod(
            "thoth_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "thoth_cli.tools_config",
        mod("thoth_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["THOTH_TUI_ACTIVE_SESSION_FILE"])
    ...
    env = captured["env"]
    assert env["THOTH_MODEL"] == "nous/hermes-test"
    assert env["THOTH_INFERENCE_MODEL"] == "nous/hermes-test"
    assert env["THOTH_TUI_PROVIDER"] == "nous"
...
def test_launch_tui_exit_code_42_relaunches_update(monkeypatch, main_mod):
    ...
    with patch("thoth_cli.relaunch.relaunch") as mock_relaunch:
        with pytest.raises(SystemExit) as exc:
            main_mod._launch_tui()
...
def test_launch_tui_drops_stale_resume_env_without_resume_arg(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("THOTH_TUI_RESUME", "stale-missing-session")
    monkeypatch.setattr(
        main_mod,
        "_make_tui_argv",
        lambda tui_dir, tui_dev: (["node", "dist/entry.js"], Path(".")),
    ...
    assert "THOTH_TUI_RESUME" not in captured["env"]
...
def test_launch_tui_sets_resume_env_from_resume_arg(monkeypatch, main_mod):
    ...
    assert captured["env"]["THOTH_TUI_RESUME"] == "20260518_000000_goodid"
...
def test_make_tui_argv_dev_prebuilds_thoth_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("THOTH_TUI_DIR", raising=False)
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    ...
    assert "thoth --tui --resume 20260409_000001_abc123" in out
    assert 'thoth --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "thoth --tui --resume actual_session" in out

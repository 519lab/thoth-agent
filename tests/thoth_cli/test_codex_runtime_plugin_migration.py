...
from thoth_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
...
class TestMigrate:
    ...
    def test_no_servers_still_writes_permissions_default(self, tmp_path):
        ...
        report = migrate({}, codex_home=tmp_path, discover_plugins=False, expose_thoth_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from thoth_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from thoth_cli.codex_runtime_plugin_migration import _query_codex_plugins
    ...
    def test_plugin_discovery_failure_non_fatal(self, tmp_path, monkeypatch):
        ...
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, discover_plugins=True, expose_thoth_tools=False)
        assert report.written
    ...
    def test_discover_plugins_false_skips_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, discover_plugins=False, expose_thoth_tools=False)
    ...
    def test_dry_run_skips_plugin_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, dry_run=True, discover_plugins=True, expose_thoth_tools=False)
        assert called["yes"] is False
    ...
    def test_expose_thoth_tools_writes_callback_mcp_entry(self, tmp_path):
        """When expose_thoth_tools=True (production default), an
        [mcp_servers.thoth-tools] entry is written so codex calls back
        into Thoth for browser/web/delegate_task/vision/memory tools.

        This is the fix for 'all other tools that codex doesn't provide
        should be useable by thoth' — quirk #7."""
        report = migrate({}, codex_home=tmp_path,
                         discover_plugins=False,
                         default_permission_profile=None,
        ...
        assert "thoth-tools" in report.migrated

    def test_expose_thoth_tools_disabled_skips_entry(self, tmp_path):
        """expose_thoth_tools=False suppresses the callback registration."""
    ...
    def test_dry_run_doesnt_write(self, tmp_path):
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, dry_run=True, expose_thoth_tools=False)
    ...
    def test_full_migration_round_trip(self, tmp_path):
        ...
        report = migrate(thoth_cfg, codex_home=tmp_path, expose_thoth_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_idempotent_re_run_replaces_managed_block(self, tmp_path):
        # First migration
        migrate({"mcp_servers": {"a": {"command": "x"}}}, codex_home=tmp_path, expose_thoth_tools=False)
        first_text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.a]" in first_text
        # Second migration with different servers
        migrate({"mcp_servers": {"b": {"command": "y"}}}, codex_home=tmp_path, expose_thoth_tools=False)
    ...
    def test_managed_root_keys_stay_top_level_when_config_ends_in_table(self, tmp_path):
        """TOML has no explicit 'leave current table' syntax. If Thoth appends
        ...
        migrate({}, codex_home=tmp_path, discover_plugins=False, expose_thoth_tools=False)
        new_text = target.read_text()
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Thoth's managed block, re-running
    ...
    def test_skipped_keys_reported(self, tmp_path):
        ...
        }, codex_home=tmp_path, expose_thoth_tools=False)
    ...
    def test_invalid_mcp_servers_value(self, tmp_path):
        report = migrate({"mcp_servers": "notadict"}, codex_home=tmp_path, expose_thoth_tools=False)
...
class TestStripUnmanagedPluginTables:
    ...
    ``thoth codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "thoth_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
        migrate({}, codex_home=tmp_path, discover_plugins=True, expose_thoth_tools=False)
        new_text = target.read_text()
...
class TestThothHomeLeakGuard:
    ...
    once codex spawned the thoth-tools MCP subprocess.
    """
    ...
    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when THOTH_HOME points at a pytest
        ...
        assert "THOTH_HOME" not in env, (
            f"pytest-tempdir THOTH_HOME leaked into codex MCP entry: "
            f"{env.get('THOTH_HOME')!r}"
        )

    def test_real_thoth_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate THOTH_HOME (not a tempdir path) DOES propagate so the
        ...
        entry = _build_thoth_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("THOTH_HOME") == real_path

    def test_unset_thoth_home_omits_env_key(self, monkeypatch):
        """When THOTH_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever THOTH_HOME its launcher (systemd, gateway, shell)
        ...
        env = entry.get("env", {})
        assert "THOTH_HOME" not in env, (
            f"THOTH_HOME should not be set when env var is unset, got: "
            f"{env.get('THOTH_HOME')!r}"
        )

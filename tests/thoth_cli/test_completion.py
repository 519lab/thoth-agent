"""Tests for thoth_cli/completion.py — shell completion script generation."""
...
from thoth_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_thoth_completion()" in out
        assert "complete -F _thoth_completion thoth" in out
...
class TestGenerateZsh:
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_thoth_profiles'" in out
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c thoth -f" in out
...
class TestSubcommandDrift:
    def test_SUBCOMMANDS_covers_required_commands(self):
        ...
        """
        import inspect
        from thoth_cli.main import _coalesce_session_name_args
...
class TestProfileCompletion:
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'thoth profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _thoth_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_thoth_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _thoth_profiles"
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__thoth_profiles" in out
        count = out.count("(__thoth_profiles)")

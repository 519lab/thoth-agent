"""Comprehensive tests for thoth_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and THOTH_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.thoth/profiles)
    * THOTH_HOME  -> tmp_path/.thoth  (so get_thoth_home() agrees)
    * Creates the bare-minimum ~/.thoth directory.
    """
...
class TestGetProfileDir:
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".thoth" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".thoth" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.thoth must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".thoth"
...
class TestNoSkillsOptOut:
    """Tests for `thoth profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `thoth update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("thoth_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".thoth" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_thoth_home_returns_default(self, profile_env):
        # THOTH_HOME points to tmp_path/.thoth which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom THOTH_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("THOTH_HOME", str(custom))
        # With Docker-aware roots, a custom THOTH_HOME is the default —
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "thoth.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["thoth.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["thoth.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "thoth.ssi_health": {"workspace": "thoth", "enabled": True}
            }
        }))
        ...
        assert "thoth.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["thoth.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["thoth.heimdall"]["workspace"] == "thoth"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "thoth.ssi_health": {"aiPeer": "ssi_health"},
                "thoth.heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["thoth.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["thoth.heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".thoth_history",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.thoth_history",
...
class TestInternalHelpers:
    ...
    def test_default_thoth_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_thoth_home() returns THOTH_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("THOTH_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (THOTH_HOME under ~/.thoth), profiles root is still ~/.thoth/profiles."""
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under THOTH_HOME."""
        from thoth_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("THOTH_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under THOTH_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("THOTH_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".thoth" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from thoth_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".thoth"

"""Tests for the `thoth proxy` subcommand and its upstream adapters."""
...
from thoth_cli.proxy.adapters.nous_portal import NousPortalAdapter
from thoth_cli.proxy.adapters.xai import XAIGrokAdapter
...
def test_nous_adapter_get_credential_uses_runtime_resolver(tmp_path, monkeypatch):
    ...
    with patch(
        "thoth_cli.proxy.adapters.nous_portal.resolve_nous_runtime_credentials",
...
def test_xai_adapter_retry_refreshes_current_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("thoth_cli.auth.refresh_xai_oauth_pure", fake_refresh)
...
from thoth_cli.proxy.server import create_app  # noqa: E402
...
def test_cmd_proxy_providers_runs(capsys):
    from thoth_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_when_unauthenticated(capsys, tmp_path, monkeypatch):
    ...
    assert rc == 2
    err = capsys.readouterr().err
    assert "thoth login nous" in err

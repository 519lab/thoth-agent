"""Tests for the ``thoth send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``thoth_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from thoth_cli import send_cmd
...
def test_load_thoth_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``thoth send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``thoth config set``) lives
    ...
    # Force get_thoth_home() to re-resolve under the patched env.

"""Tests for xAI Grok OAuth — tokens stored in Thoth auth store (~/.hermes/auth.json)."""
...
def _setup_thoth_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the Thoth auth store at the given root."""
    ...
    auth_file = thoth_home / "auth.json"
...
def _patch_httpx_client(monkeypatch, response):
    ...
    monkeypatch.setattr("thoth_cli.auth.httpx.Client", _factory)
    return holder
...
def test_xai_oauth_authorize_url_includes_referrer_thoth_agent():
    ...
    params = _parse_authorize_url(url)
    assert params["referrer"] == "thoth-agent"
...
def test_xai_callback_handler_returns_400_when_callback_url_lacks_code_and_error():
    ...
    try:
        ...
        assert "thoth auth add xai-oauth" in body
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("thoth_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
def test_resolve_xai_runtime_credentials_honours_env_base_url(tmp_path, monkeypatch):
    ...
    _setup_thoth_auth(thoth_home, access_token=fresh)
    monkeypatch.setenv("THOTH_HOME", str(thoth_home))
    monkeypatch.setenv("THOTH_XAI_BASE_URL", "https://custom.x.ai/v1/")
...
# ``XAI_BASE_URL`` / ``THOTH_XAI_BASE_URL`` are a credential-leak vector
...
def _seed_xai_oauth_state(
    thoth_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    thoth_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
    ...
    (thoth_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    monkeypatch.setattr("thoth_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
    ...
    raw = json.loads((thoth_home / "auth.json").read_text())
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    monkeypatch.setattr("thoth_cli.auth._refresh_xai_oauth_tokens", _transient_refresh)
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``thoth model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``thoth model``
    ...
    assert "thoth model" not in rendered.lower()
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from thoth_cli.auth import _xai_oauth_discovery
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    Thoth that didn't validate) must not be silently honored on the refresh
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("thoth_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `thoth model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `thoth auth list` reflects truth and
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    """`thoth auth remove xai-oauth <N>` for the seeded entry suppresses
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``thoth auth remove xai-oauth 1`` for a
    ...
    from agent.credential_pool import load_pool
    from thoth_cli.auth_commands import auth_remove_command
    ...
    # Act: the user runs `thoth auth remove xai-oauth 1`.
    ...
    # nothing to import.
    raw_after = json.loads((thoth_home / "auth.json").read_text())
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every Thoth invocation."
    )
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("thoth_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
...
def test_pool_entry_needs_refresh_when_jwt_within_skew(tmp_path, monkeypatch):
    ...
    from agent.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    from thoth_cli.auth import XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
    import uuid
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("thoth_cli.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    """Multi-process race: another Thoth process refreshed the singleton
    ...
    (thoth_home / "auth.json").write_text(json.dumps(raw))
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``thoth model`` -> xAI Grok OAuth (or another process
    ...
    # Simulate the user re-running `thoth model` -> xAI Grok OAuth: the
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``thoth auth add xai-oauth``) own their own refresh-token lifecycle
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`thoth auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``thoth auth status``, ``thoth setup``, and the
    ``thoth`` no-arg dispatcher).  Pin the ``set_active=False`` contract so

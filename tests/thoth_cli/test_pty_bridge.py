"""Unit tests for thoth_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf thoth-ok"])
        try:
            output = _read_until(bridge, b"thoth-ok")
            assert b"thoth-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$THOTH_PTY_TEST\""],
            env={**os.environ, "THOTH_PTY_TEST": "pty-env-works"},
        )
        try:

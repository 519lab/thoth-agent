...
Covers:
 - Normal path (tokens already in Thoth auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Thoth
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `thoth auth openai-codex`. The old
...
def test_normal_path_still_works(thoth_auth_only_env):
    """openai-codex appears when tokens are already in Thoth auth store."""
...
def test_codex_picker_uses_live_codex_catalog(thoth_auth_only_env, tmp_path, monkeypatch):

"""Tests for Codex auth — tokens stored in Thoth auth store (~/.hermes/auth.json)."""
...
def _setup_thoth_auth(thoth_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Thoth auth store."""
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_thoth_auth(thoth_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("THOTH_HOME", str(thoth_home))
    ...
    monkeypatch.setattr("thoth_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    thoth_home = tmp_path / "hermes"
    _setup_thoth_auth(thoth_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("THOTH_HOME", str(thoth_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Thoth auth store, not ~/.codex/."""
    thoth_home = tmp_path / "hermes"
    codex_home = tmp_path / "codex-cli"
    thoth_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr("thoth_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("thoth_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",

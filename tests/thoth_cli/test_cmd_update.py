...
from thoth_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_refreshes_repo_and_web_node_dependencies(
        ...
        from thoth_cli import main as hm
        ...
        #   1. repo root  — slash-command bridge deps
        ...
        # (capture_output=False) so postinstall progress is visible to
        # the user.
        repo_calls = [
            call
            for call in mock_run.call_args_list
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "thoth_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "thoth_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("thoth_cli.config.check_config_version", return_value=(1, 2)), patch(
            "thoth_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("thoth_cli.main.sys") as mock_sys:
            ...
            from thoth_cli.config import migrate_config
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.thoth"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.thoth/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.thoth/profiles/work"))
        ...
        with (
            patch("thoth_cli.profiles.list_profiles", return_value=all_profiles),
            patch("thoth_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("thoth_cli.profiles.list_profiles", return_value=[default_p]),
            patch("thoth_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)

...
- Bare `thoth setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `thoth setup --quick` runs the narrower "fill in missing items" flow.
- `thoth setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("thoth_cli.setup.ensure_hermes_home", {}),
        ("thoth_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("thoth_cli.config.is_managed", {"return_value": False}),
        ("thoth_cli.setup.load_config", {"return_value": {}}),
        ("thoth_cli.setup.save_config", {}),
        ("thoth_cli.setup.get_env_value", {"return_value": None}),
        ("thoth_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("thoth_cli.setup._print_setup_summary", {}),
        ("thoth_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("thoth_cli.setup.load_config", {"return_value": {}}),
        ("thoth_cli.setup.save_config", {}),
        ("thoth_cli.auth.get_active_provider", {"return_value": None}),
        ("thoth_cli.setup.get_env_value", {"return_value": None}),
        ("thoth_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `thoth setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="thoth_cli.setup.prompt_choice",
                quick="thoth_cli.setup._run_quick_setup",
                model="thoth_cli.setup.setup_model_provider",
                terminal="thoth_cli.setup.setup_terminal_backend",
                agent="thoth_cli.setup.setup_agent_settings",
                gateway="thoth_cli.setup.setup_gateway",
                tools="thoth_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`thoth setup --reconfigure` behaves the same as bare `thoth setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("thoth_cli.setup.prompt_choice", {"return_value": 0}),
                first="thoth_cli.setup._run_first_time_quick_setup",
            )
            from thoth_cli.setup import run_setup_wizard
            run_setup_wizard(args)

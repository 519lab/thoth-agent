...
normal tests assume away:
  ...
  - Filesystem: THOTH_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own THOTH_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            home = tempfile.mkdtemp(prefix=f"thoth_atyp_{name}_")
            os.environ["THOTH_HOME"] = home
            os.environ["HOME"] = home
            for m in list(sys.modules.keys()):
                if m.startswith(("thoth_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from thoth_cli import kanban_db as kb  # noqa: F401
            print(f"\n═══ {name} ═══")
            try:
                fn(home, kb)
                print(f"  ✔ {name}")
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "THOTH_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from thoth_cli.kanban_db import resolve_workspace
            ...
            home_abs = str(Path(os.environ["THOTH_HOME"]).resolve())
            if not resolved_abs.startswith(home_abs) and resolved_abs.startswith("/tmp"):
                ...
                print(f"  ⚠ workspace resolved OUTSIDE thoth_home: {resolved}")
...
@scenario("thoth_home_with_spaces")
def _(home, kb):
    """THOTH_HOME at a path with spaces — should work but catches
    ...
    weird = tempfile.mkdtemp(prefix="thoth with spaces ")
    ...
    try:
        ...
        print(f"  THOTH_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("thoth_home_with_unicode")
def _(home, kb):
    ...
    try:
        ...
        print(f"  THOTH_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("thoth_home_via_symlink")
def _(home, kb):
    """THOTH_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    uses Path.resolve() — two different symlink names pointing at the
    same dir should NOT double-init."""
    real = tempfile.mkdtemp(prefix="thoth_real_")
    link1 = real + "_link1"
    link2 = real + "_link2"
    os.symlink(real, link1)
    os.symlink(real, link2)
    try:
        os.environ["THOTH_HOME"] = link1
        ...
        conn2.close()
        print("  symlinks to same THOTH_HOME share DB correctly")
    finally:
...
def _idempotency_race_worker(thoth_home: str, key: str, result_file: str,
...
def _(home, kb):
    ...
    try:
        from thoth_cli import web_server as ws  # noqa
    except Exception:
        pass

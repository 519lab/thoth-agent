...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves thoth_cli.kanban_db on its own
  ...
  - worker logs are captured to THOTH_HOME/kanban/logs/<task>.log
...
def main():
    home = tempfile.mkdtemp(prefix="thoth_e2e_")
    os.environ["THOTH_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from thoth_cli import kanban_db as kb

    # Point the `thoth` CLI child processes will run at the worktree
    # thoth_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "thoth")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m thoth_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/thoth_e2e_*/worker_*.log
    import glob

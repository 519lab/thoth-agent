...
Key invariants covered by these tests:
  ...
  5. Load order: every Thoth entry point imports thoth_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported thoth_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("thoth_bootstrap", None)
    import thoth_bootstrap  # noqa: WPS433
    return thoth_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Thoth CLI
...
class TestEntryPointsImportBootstrap:
    """Every Thoth entry point must import thoth_bootstrap as its
    ...
    # Entry points that invoke Thoth as a process.  Each one must
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import thoth_bootstrap' and that
        ...
        half-finished ``thoth update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``thoth_bootstrap`` as a top-level module) leaves thoth
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # thoth start even when thoth_bootstrap hasn't been
        ...
        assert first_import_name == "thoth_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'thoth_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import thoth_bootstrap' line to be the first import."
        )

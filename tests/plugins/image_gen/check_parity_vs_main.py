...
# worktree), ``MAIN_DIR`` falls back to a sibling ``thoth-agent-main``
...
def _resolve_main_dir() -> Path:
    ...
    if (candidate / "tools" / "image_generation_tool.py").exists() and candidate != REPO_ROOT:
        return candidate
    sibling = REPO_ROOT.parent / "thoth-agent-main"
...
# Isolated THOTH_HOME so the config write is hermetic.
...
def main() -> int:
    ...
    if MAIN_DIR == PR_DIR:
        print(
            ...
            "      Set up a sibling 'thoth-agent-main' checkout pinned to "

...
Run from the PR worktree:
    ...
    cd ~/.thoth/thoth-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.thoth/thoth-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.thoth/thoth-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a thoth-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a thoth-agent checkout"
)
...
# Isolated THOTH_HOME for the config write.

...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Thoth passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("THOTH_HOME", "/users/alice/.thoth/profiles/backend-worker")
        monkeypatch.setenv("THOTH_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "THOTH_KANBAN_DB",
            "/users/alice/.thoth/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.thoth/kanban/boards/smoke"]'
            in cmd
        )

...
def _patch_oauth_flow(
    ...
    *,
    ...
    """Wire up monkeypatches that let ``run_thoth_oauth_login_pure()`` run
...
def test_authorization_url_state_is_not_pkce_verifier(monkeypatch, tmp_path):
    ...
    from agent.anthropic_adapter import run_thoth_oauth_login_pure

    result = run_thoth_oauth_login_pure()

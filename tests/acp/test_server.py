...
from acp_adapter.server import ThothACPAgent, THOTH_VERSION
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "thoth-agent"
        assert resp.agent_info.version == THOTH_VERSION
        ...
        assert payloads == [
            {
                "args": ["--setup"],
                "description": (
                    "Open Thoth's interactive model/provider setup in a terminal. "
                    "Use this when Thoth has not been configured on this machine yet."
                ),
                "id": TERMINAL_SETUP_AUTH_METHOD_ID,
                "name": "Configure Thoth provider",
                "type": "terminal",
            }
        ]
...
class TestPrompt:
    ...
    async def test_prompt_propagates_thoth_session_id_env(self, agent, monkeypatch):
        ...
        via ``THOTH_SESSION_ID`` so tools that want to stamp side-effects
        ...
        def mock_run(user_message, conversation_history=None, task_id=None, **kwargs):
            ...
            captured["env"] = os.environ.get("THOTH_SESSION_ID")
        ...
        assert captured["env"] == new_resp.session_id, (
            "THOTH_SESSION_ID must be set to the originating ACP session id "
        ...
        assert os.environ.get("THOTH_SESSION_ID") is None, (
            "THOTH_SESSION_ID must be restored after the agent call so "
    ...
    async def test_prompt_restores_prior_thoth_session_id(self, agent, monkeypatch):
        ...
        def mock_run(*args, **kwargs):
            captured["inner"] = os.environ.get("THOTH_SESSION_ID")
            return {"final_response": "ok", "messages": []}
        ...
        assert os.environ.get("THOTH_SESSION_ID") == "outer-sess"
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert THOTH_VERSION in result
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["thoth-acp"]
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        ...
        mock_defs.assert_called_once_with(
            enabled_toolsets=["thoth-acp", "mcp-srv"],
            disabled_toolsets=None,
            quiet_mode=True,
        )
        assert state.agent.enabled_toolsets == ["thoth-acp", "mcp-srv"]

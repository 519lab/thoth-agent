"""Tests for ACP Registry metadata shipped with Thoth."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "thoth-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Thoth Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/519lab/thoth-agent"
    assert data["website"].startswith("https://thoth.519lab.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"thoth-agent[acp]=={data['version']}"
    assert uvx["args"] == ["thoth-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"thoth-agent[acp]=={_pyproject_version()}"
    )

...
def cron_env(tmp_path, monkeypatch):
    """Isolated THOTH_HOME with an empty skills tree.

    `tools.skills_tool` snapshots `SKILLS_DIR` at module-import time, so
    setting `THOTH_HOME` alone doesn't reach it. We also patch the
...
class TestBuildJobPromptScansSkillContent:
    def test_clean_skill_builds_normally(self, cron_env):
        thoth_home, scheduler = cron_env
        _plant_skill(thoth_home, "news-digest", "Fetch the top 5 headlines and summarize.")
    ...
    def test_skill_with_invisible_unicode_raises(self, cron_env):
        ...
        _plant_skill(thoth_home, "zwsp-skill", "clean looking\u200bskill content")

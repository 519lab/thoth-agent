# Thoth Agent v0.2.0 (v2026.3.12)

**Release Date:** March 12, 2026

> First tagged release since v0.1.0 (the initial pre-public foundation). In just over two weeks, Thoth Agent went from a small internal project to a full-featured AI agent platform — thanks to an explosion of community contributions. This release covers **216 merged pull requests** from **63 contributors**, resolving **119 issues**.
...
- **Git Worktree Isolation** — `thoth -w` launches isolated agent sessions in git worktrees for safe parallel work on the same repo. ([#654](https://github.com/NousResearch/hermes-agent/pull/654))
...
- `thoth tools` — per-platform tool enable/disable with curses UI
- `thoth doctor` for health checks across all configured providers
- `thoth update` with auto-restart for gateway service
...
- Consistent THOTH_HOME and .env path resolution ([#51](https://github.com/NousResearch/hermes-agent/pull/51), [#48](https://github.com/NousResearch/hermes-agent/pull/48)) — @deankerr
...
- `thoth skills browse` — paginated hub browsing
...
- **Thoth-Atropos** — RL environment development skill ([#815](https://github.com/NousResearch/hermes-agent/pull/815))
...
- Fix provider selection not persisting when switching via thoth model ([#881](https://github.com/NousResearch/hermes-agent/pull/881))
...
- Fix install.sh creating ~/.thoth before moving Node.js directory ([#53](https://github.com/NousResearch/hermes-agent/pull/53)) — @JoshuaMart
...
- Thoth-atropos-environments bundled skill ([#815](https://github.com/NousResearch/hermes-agent/pull/815))
...
Thank you to the 63 contributors who made this release possible! In just over two weeks, the Thoth Agent community came together to ship an extraordinary amount of work.

...
- **Camofox Anti-Detection Browser Backend** — New local browser backend using Camoufox for stealth browsing. Persistent sessions with VNC URL discovery for visual debugging, configurable SSRF bypass for local backends, auto-install via `thoth tools`. ([#4008](https://github.com/NousResearch/hermes-agent/pull/4008), [#4419](https://github.com/NousResearch/hermes-agent/pull/4419), [#4292](https://github.com/NousResearch/hermes-agent/pull/4292))
...
- **API Server Session Continuity & Tool Streaming** — The API server (Open WebUI integration) now streams tool progress events in real-time and supports `X-Thoth-Session-Id` headers for persistent sessions across requests. Sessions persist to the shared SessionDB. ([#4092](https://github.com/NousResearch/hermes-agent/pull/4092), [#4478](https://github.com/NousResearch/hermes-agent/pull/4478), [#4802](https://github.com/NousResearch/hermes-agent/pull/4802))

- **ACP: Client-Provided MCP Servers** — Editor integrations (VS Code, Zed, JetBrains) can now register their own MCP servers, which Thoth picks up as additional agent tools. Your editor's MCP ecosystem flows directly into the agent. ([#4705](https://github.com/NousResearch/hermes-agent/pull/4705))
...
- **MiniMax M2.7** added to thoth model picker and OpenCode ([#4208](https://github.com/NousResearch/hermes-agent/pull/4208))
...
- **Unified setup wizard provider selection** with `thoth model` — single code path for both flows ([#4200](https://github.com/NousResearch/hermes-agent/pull/4200))
...
- **THOTH_HOME remapped** to target user in system service unit ([#4456](https://github.com/NousResearch/hermes-agent/pull/4456))
...
- **`--max-turns` CLI flag** for `thoth chat` to limit agent iterations ([#4314](https://github.com/NousResearch/hermes-agent/pull/4314))
...
- **Claude Code credentials gated** behind explicit Thoth config in wizard trigger ([#4210](https://github.com/NousResearch/hermes-agent/pull/4210))
...
### Update System
- **Fork detection and upstream sync** in `thoth update` ([#4744](https://github.com/NousResearch/hermes-agent/pull/4744))
...
- **Handle conflicted git index** during thoth update ([#4735](https://github.com/NousResearch/hermes-agent/pull/4735))
...
### Browser
- **Camofox anti-detection browser backend** — local stealth browsing with auto-install via `thoth tools` ([#4008](https://github.com/NousResearch/hermes-agent/pull/4008))
...
- **Client-provided MCP servers** registered as agent tools — editors pass their MCP servers to Thoth ([#4705](https://github.com/NousResearch/hermes-agent/pull/4705))
...
- **Unified thoth-agent and thoth-agent-setup** into single skill ([#4332](https://github.com/NousResearch/hermes-agent/pull/4332))
...
- **MCP OAuth server** could block Thoth startup instead of degrading gracefully ([#4757](https://github.com/NousResearch/hermes-agent/pull/4757), closes [#4462](https://github.com/NousResearch/hermes-agent/issues/4462))

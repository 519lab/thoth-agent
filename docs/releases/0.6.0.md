...
- **Profiles — Multi-Instance Thoth** — Run multiple isolated Thoth instances from the same installation. Each profile gets its own config, memory, sessions, skills, and gateway service. Create with `thoth profile create`, switch with `thoth -p <name>`, export/import for sharing. Full token-lock isolation prevents two profiles from using the same bot credential. ([#3681](https://github.com/NousResearch/hermes-agent/pull/3681))

- **MCP Server Mode** — Expose Thoth conversations and sessions to any MCP-compatible client (Claude Desktop, Cursor, VS Code, etc.) via `thoth mcp serve`. Browse conversations, read messages, search across sessions, and manage attachments — all through the Model Context Protocol. Supports both stdio and Streamable HTTP transports. ([#3795](https://github.com/NousResearch/hermes-agent/pull/3795))

- **Docker Container** — Official Dockerfile for running Thoth Agent in a container. Supports both CLI and gateway modes with volume-mounted config. ([#3668](https://github.com/NousResearch/hermes-agent/pull/3668), closes [#850](https://github.com/NousResearch/hermes-agent/issues/850))

- **Ordered Fallback Provider Chain** — Configure multiple inference providers with automatic failover. When your primary provider returns errors or is unreachable, Thoth automatically tries the next provider in the chain. Configure via `fallback_providers` in config.yaml. ([#3813](https://github.com/NousResearch/hermes-agent/pull/3813), closes [#1734](https://github.com/NousResearch/hermes-agent/issues/1734))
...
- **Slack Multi-Workspace OAuth** — Connect a single Thoth gateway to multiple Slack workspaces via OAuth token file. Each workspace gets its own bot token, resolved dynamically per incoming event. ([#3903](https://github.com/NousResearch/hermes-agent/pull/3903))
...
- **Fix api_mode on provider switch** — switching providers via `thoth model` now correctly clears stale `api_mode` instead of hardcoding `chat_completions`, fixing 404s for providers with Anthropic-compatible endpoints ([#3726](https://github.com/NousResearch/hermes-agent/pull/3726), [#3857](https://github.com/NousResearch/hermes-agent/pull/3857), closes [#3685](https://github.com/NousResearch/hermes-agent/issues/3685))
- **Stop silent OpenRouter fallback** — when no provider is configured, Thoth now raises a clear error instead of silently routing to OpenRouter ([#3807](https://github.com/NousResearch/hermes-agent/pull/3807), [#3862](https://github.com/NousResearch/hermes-agent/pull/3862))
...
- **Profiles system** — `thoth profile create/list/switch/delete/export/import/rename`. Each profile gets isolated THOTH_HOME, gateway service, CLI wrapper. Token locks prevent credential collisions. Tab completion for profile names. ([#3681](https://github.com/NousResearch/hermes-agent/pull/3681))
- **Profile-aware display paths** — all user-facing `~/.thoth` paths replaced with `display_thoth_home()` to show the correct profile directory ([#3623](https://github.com/NousResearch/hermes-agent/pull/3623))
- **Lazy display_thoth_home imports** — prevents `ImportError` during `thoth update` when modules cache stale bytecode ([#3776](https://github.com/NousResearch/hermes-agent/pull/3776))
- **THOTH_HOME for protected paths** — `.env` write-deny path now respects THOTH_HOME instead of hardcoded `~/.thoth` ([#3840](https://github.com/NousResearch/hermes-agent/pull/3840))
...
- **DISCORD_IGNORE_NO_MENTION** — skip messages that @mention other users/bots but not Thoth ([#3640](https://github.com/NousResearch/hermes-agent/pull/3640))
...
- **Tool token context display** — `thoth tools` checklist now shows estimated token cost per toolset ([#3805](https://github.com/NousResearch/hermes-agent/pull/3805))
...
### Setup & Configuration
- **Auto-install matrix-nio** during `thoth setup` when Matrix is selected ([#3802](https://github.com/NousResearch/hermes-agent/pull/3802), [#3873](https://github.com/NousResearch/hermes-agent/pull/3873))
...
- **Clear __pycache__ during update** — prevents stale bytecode ImportError after `thoth update` ([#3819](https://github.com/NousResearch/hermes-agent/pull/3819))
...
### MCP
- **MCP Server Mode** — `thoth mcp serve` exposes conversations, sessions, and attachments to MCP clients via stdio or Streamable HTTP ([#3795](https://github.com/NousResearch/hermes-agent/pull/3795))
...
### Plugin System
- **Plugin enable/disable commands** — `thoth plugins enable/disable <name>` for managing plugin state without removing them ([#3747](https://github.com/NousResearch/hermes-agent/pull/3747))
...
- **Lazy imports for update safety** — prevent ImportError chains during `thoth update` when modules reference new functions ([#3776](https://github.com/NousResearch/hermes-agent/pull/3776))
...
- **api_mode stale on provider switch** — correctly clear when switching providers via `thoth model` ([#3857](https://github.com/NousResearch/hermes-agent/pull/3857))
...
- **Comprehensive OpenClaw migration guide** — step-by-step guide for migrating from OpenClaw/Claw3D to Thoth Agent ([#3864](https://github.com/NousResearch/hermes-agent/pull/3864), [#3900](https://github.com/NousResearch/hermes-agent/pull/3900))

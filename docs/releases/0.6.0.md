...
- **Profiles system** — `hermes profile create/list/switch/delete/export/import/rename`. Each profile gets isolated THOTH_HOME, gateway service, CLI wrapper. Token locks prevent credential collisions. Tab completion for profile names. ([#3681](https://github.com/NousResearch/hermes-agent/pull/3681))
...
- **THOTH_HOME for protected paths** — `.env` write-deny path now respects THOTH_HOME instead of hardcoded `~/.hermes` ([#3840](https://github.com/NousResearch/hermes-agent/pull/3840))

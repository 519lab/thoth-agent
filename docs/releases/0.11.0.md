# Thoth Agent v0.11.0 (v2026.4.23)
...
- **Shell hooks** — Wire any shell script as a Thoth lifecycle hook (pre_tool_call, post_tool_call, on_session_start, etc.) without writing a Python plugin. ([#13296](https://github.com/NousResearch/hermes-agent/pull/13296))
...
- Fix: discover user-installed memory providers from `$THOTH_HOME/plugins/` ([#10529](https://github.com/NousResearch/hermes-agent/pull/10529))
...
A full React/Ink rewrite of the interactive CLI — invoked via `thoth --tui` or `THOTH_TUI=1`. Shipped across ~310 commits to `ui-tui/` and `tui_gateway/`.
...
### Image Generation
- **Multi-model FAL support** with picker in `thoth tools` ([#11265](https://github.com/NousResearch/hermes-agent/pull/11265))
...
- **`thoth skills reset`** to un-stick bundled skills ([#11468](https://github.com/NousResearch/hermes-agent/pull/11468))
...
- **Doctor: Command Installation check** for `thoth` bin symlink ([#10112](https://github.com/NousResearch/hermes-agent/pull/10112))
- **ESC cancels secret/sudo prompts**, clearer skip messaging ([#9902](https://github.com/NousResearch/hermes-agent/pull/9902))
- Fix: agent-facing text uses `display_thoth_home()` instead of hardcoded `~/.thoth` ([#10285](https://github.com/NousResearch/hermes-agent/pull/10285))
- Fix: enforce `config.yaml` as sole CWD source + deprecate `.env` CWD vars + add `thoth memory reset` ([#11029](https://github.com/NousResearch/hermes-agent/pull/11029))
...
- **Warn about legacy thoth.service units** during `thoth update` ([#11918](https://github.com/NousResearch/hermes-agent/pull/11918))
...
- Trajectory compressor credentials load from `THOTH_HOME/.env` ([#9632](https://github.com/NousResearch/hermes-agent/pull/9632), @Dusk1e)

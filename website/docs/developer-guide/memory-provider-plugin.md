...
description: "How to build a memory provider plugin for Thoth Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Thoth Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/docs/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `thoth plugins`.
:::
...
`get_config_schema()` returns a list of field descriptors used by `thoth memory setup`:
...
Every field in `get_config_schema()` is prompted during `thoth memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$THOTH_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `thoth my-provider status`, `thoth my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `thoth <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `thoth --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: thoth my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the thoth my-provider argparse tree.

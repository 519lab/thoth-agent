...
| `thoth model` picker | `hermes_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `thoth doctor` | `hermes_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `thoth setup` | `hermes_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
...
See [Building a Thoth Plugin](/docs/guides/build-a-hermes-plugin#distribute-via-pip) for the full entry-points setup.
...
- [Building a Thoth Plugin](/docs/guides/build-a-hermes-plugin) — general plugin authoring

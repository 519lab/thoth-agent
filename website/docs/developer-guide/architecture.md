...
sidebar_position: 1
title: "Architecture"
description: "Thoth Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Thoth Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ thoth_state.py   │              │ Browser (5 backends)  │
...
├── thoth_constants.py       # HERMES_HOME, profile-aware paths
...
│
├── thoth_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `thoth` subcommands (large file)
...
│   ├── skills_config.py      # thoth skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # thoth tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # thoth gateway start/stop
│
...
Exposes Thoth as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`thoth -p <name>`) gets its own HERMES_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |

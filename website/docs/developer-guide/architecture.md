...
├── thoth_constants.py       # THOTH_HOME, profile-aware paths
...
| **Profile isolation** | Each profile (`thoth -p <name>`) gets its own THOTH_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |

...
description: "How to build an image-generation backend plugin for Thoth Agent"
---
...
The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `thoth tools`.
...
class MyBackendImageGenProvider(ImageGenProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `thoth tools`. Defaults to name.title() if omitted.
        return "My Backend"
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        # Catalog shown in `thoth tools` model picker.
        return [
            {
                "id": "my-model-fast",
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        # Metadata for the `thoth tools` picker — keys to prompt for at setup.
        return {
            "name": "My Backend",
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            #   - base64 data: save under $THOTH_HOME/cache/images/ via save_b64_image()
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `thoth plugins install`.
...
| `display_name` | — | `name.title()` | Label shown in `thoth tools` |
| `is_available()` | — | `True` | Gate for missing creds/deps |
| `list_models()` | — | `[]` | Catalog for `thoth tools` model picker |
...
thoth plugins enable my-backend

# Pick it as the active provider
echo "image_gen:" >> $THOTH_HOME/config.yaml
echo "  provider: my-backend" >> $THOTH_HOME/config.yaml

# Exercise it
thoth -z "Generate an image of a corgi in a spacesuit"
```

Or interactively: `thoth tools` → "Image Generation" → select `my-backend` → enter API key if prompted.

...
description: "How Thoth stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `thoth_cli/cron.py` | CLI `thoth cron` subcommands |
...
Jobs are stored in `~/.thoth/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `thoth cron` commands are run or during active CLI sessions.
...
```python
# ~/.thoth/scripts/check_competitors.py
...
2. **Environment variable** — `THOTH_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.thoth/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `thoth cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `thoth cron` CLI provides direct job management:

```bash
thoth cron list                    # Show all jobs
thoth cron create                  # Interactive job creation (alias: add)
thoth cron edit <job_id>           # Edit job configuration
thoth cron pause <job_id>          # Pause a running job
thoth cron resume <job_id>         # Resume a paused job
thoth cron run <job_id>            # Trigger immediate execution
thoth cron remove <job_id>         # Delete a job
```

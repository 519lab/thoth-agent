...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Thoth already has applies. The plugin never sees a
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Thoth provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Thoth provider
        # (must be one Thoth already knows about — same names as
        # `thoth model` and config.yaml model.provider).
        ...
        # plugin sends — Thoth does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.thoth/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Thoth's usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Thoth subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/519lab/thoth-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/519lab/thoth-agent/blob/main/tests/agent/test_plugin_llm.py)

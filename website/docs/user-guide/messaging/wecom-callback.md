...
Connect Thoth to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Thoth supports two WeCom integration modes:
...
3. Thoth decrypts the message, queues it for the agent
...
(Use `thoth gateway start` only after `thoth gateway install` has registered the systemd/launchd service.)

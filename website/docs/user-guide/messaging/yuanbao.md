...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Thoth Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Thoth to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.thoth/.env`:
...
All standard Thoth commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.thoth/logs/gateway.log`
...
4. Consider enabling verbose logging: `THOTH_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Thoth automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
THOTH_LOG_LEVEL=debug thoth gateway
```
...
```bash
thoth chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```

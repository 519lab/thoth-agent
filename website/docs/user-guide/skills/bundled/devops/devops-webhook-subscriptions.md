...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Thoth agent runs by POSTing events to a URL.
...
All management is via the `thoth webhook` CLI command:

### Create a subscription
```bash
thoth webhook subscribe <name> \
...
### GitHub: PR reviews
```bash
thoth webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
thoth webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
thoth webhook subscribe alerts \
...
- Subscriptions persist to `~/.thoth/webhook_subscriptions.json`

## How It Works

1. `thoth webhook subscribe` writes to `~/.thoth/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status thoth-gateway` or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.thoth/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `thoth webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `thoth webhook test <name>` to verify the route works.

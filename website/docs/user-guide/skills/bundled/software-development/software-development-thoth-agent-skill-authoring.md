...
| Path | `skills/software-development/thoth-agent-skill-authoring` |
| Version | `1.0.0` |
| Author | Thoth Agent |
| License | MIT |
...
1. **User-local:** `~/.thoth/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
...
- Thoth-specific recipes (tests via scripts/run_tests.sh, gateway/CLI paths, etc.)
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.thoth/skills/`, not the repo tree. Use `write_file` for in-repo creation.

...
| Related skills | [`systematic-debugging`](/docs/user-guide/skills/bundled/software-development/software-development-systematic-debugging), [`node-inspect-debugger`](/docs/user-guide/skills/bundled/software-development/software-development-node-inspect-debugger), [`debugging-thoth-tui-commands`](/docs/user-guide/skills/bundled/software-development/software-development-debugging-thoth-tui-commands) |
...
- A long-running process (thoth gateway, tui_gateway) misbehaves and you can't restart it
...
For long-lived processes: Thoth gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.
...
### `tui_gateway` subprocess (spawned by `thoth --tui`)
...
debugpy.wait_for_client()
```
Start `thoth --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.

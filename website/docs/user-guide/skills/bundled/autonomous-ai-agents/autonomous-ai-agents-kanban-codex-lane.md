...
description: "Use when a Thoth Kanban worker wants to run Codex CLI as an isolated implementation lane while Thoth keeps ownership of task lifecycle, reconciliation, testi..."
---
...
Use when a Thoth Kanban worker wants to run Codex CLI as an isolated implementation lane while Thoth keeps ownership of task lifecycle, reconciliation, testing, and handoff.
...
| Related skills | [`kanban-worker`](/docs/user-guide/skills/bundled/devops/devops-kanban-worker), [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`thoth-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-thoth-agent) |
...
This skill defines the lightweight Thoth+Codex dual-lane convention for Kanban workers. Thoth is always the task owner: it calls `kanban_show`, decides whether Codex is appropriate, creates or selects an isolated workspace, starts and monitors Codex, reconciles any diff, runs verification, and writes the final `kanban_complete` or `kanban_block` handoff. Codex is an input lane only. Codex output is not a task completion signal, not a trusted reviewer, and not allowed to write durable Kanban state directly.

The convention exists so a Thoth worker can use Codex for bounded implementation help without changing the dispatcher. The dispatcher must still spawn Thoth workers. A worker may optionally spawn Codex inside its own run, then accept, partially accept, or reject the lane after independent review and tests.
...
- A bounded diff can be evaluated by Thoth in one run.
- The repo can be copied or checked out in an isolated git worktree/branch.
- Thoth can run the relevant tests itself after Codex exits.
...
1. Thoth owns the Kanban lifecycle. Codex must never call `kanban_complete`, `kanban_block`, `kanban_create`, gateway messaging, or any Thoth board CLI as a substitute for the worker.
2. Thoth owns final acceptance. Treat Codex commits/diffs as untrusted patches until reviewed and verified.
3. Thoth owns test execution. Codex may run tests, but those runs are advisory; repeat required verification from Thoth with the repo's canonical wrapper.
4. Thoth owns safety. If Codex changes safety boundaries, risk gates, live trading behavior, or secrets handling, reject the lane even if tests pass.
5. Thoth owns cleanup. Kill stuck Codex processes and remove temporary worktrees when they are no longer needed.
...
```bash
TASK_ID="${THOTH_KANBAN_TASK:-t_manual}"
REPO="/path/to/repo"
...
If the current Kanban workspace is already an isolated git worktree created for this task, you may create a sibling Codex branch inside it only if `git status --short` is clean except for intentional Thoth edits. Otherwise create a separate temporary worktree and cherry-pick or copy accepted commits back after reconciliation.
...
Run these before spawning Codex. Missing Codex is a normal reason to skip the lane, not a task blocker if Thoth can do the task directly.
...
Thoth owns the Kanban lifecycle; do not call Thoth kanban tools or messaging.
...
Do not use `--yolo` for prediction-market-bot or safety-sensitive repos. Prefer `--full-auto` inside the isolated worktree, then rely on Thoth reconciliation.
...
- Explicit statement: Thoth owns Kanban lifecycle; Codex is an input lane only.
...
- Verification commands Codex may run and commands Thoth will run afterward.
...
Thoth must perform this checklist before accepting any Codex lane result:

- [ ] `git -C <WORKTREE> status --short --branch` shows only expected files.
- [ ] `git -C <WORKTREE> diff --stat` and `git diff` were reviewed by Thoth.
...
- [ ] Thoth ran the canonical tests itself, using `scripts/run_tests.sh` for Thoth Agent or the repo's documented wrapper for other repos.
- [ ] Any Codex-run tests are listed separately from Thoth-run tests.
- [ ] Accepted commits/diffs were applied to the Thoth-owned workspace/branch.
...
```json
{
  "codex_lane": {
    ...
    "tests_run": [
      {"command": "scripts/run_tests.sh tests/tools/test_x.py", "exit_code": 0, "owner": "thoth"},
...
```json
{
  "codex_lane": {
    ...
    "result": "rejected",
    "accepted_commits": [],
    "rejected_reason": "Direct Thoth edit was smaller and safer than spawning Codex.",
    "tests_run": [],
    "artifacts": []
  }
}
...
1. Treating Codex self-report as verification. Always inspect the diff and rerun tests from Thoth.
2. Running Codex in the user's dirty main checkout. Always isolate in a worktree/branch.
3. Letting Codex own Kanban. Codex may summarize progress, but Thoth writes board state.
...
- [ ] Thoth reviewed `git diff` and safety-sensitive files.
- [ ] Thoth ran canonical tests independently.

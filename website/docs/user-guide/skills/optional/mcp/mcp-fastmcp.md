...
| | |
|---|---|
| Source | Optional — install with `thoth skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Thoth or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Thoth. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.thoth/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.thoth/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Thoth integration, either:

- configure the server in `~/.thoth/config.yaml` using the `native-mcp` skill, or
...
### Thoth cannot see the deployed server

The server-building part may be correct while the Thoth config is not. Load the `native-mcp` skill and configure the server in `~/.thoth/config.yaml`, then restart Thoth.

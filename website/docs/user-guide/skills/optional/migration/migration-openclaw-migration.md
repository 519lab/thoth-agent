---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Thoth Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Thoth Agent"
---
...
Migrate a user's OpenClaw customization footprint into Thoth Agent. Imports Thoth-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `thoth skills install official/migration/openclaw-migration` |
...
# OpenClaw -> Thoth Migration

Use this skill when a user wants to move their OpenClaw setup into Thoth Agent with minimal manual cleanup.
...
```bash
thoth claw migrate              # Full interactive migration
...
thoth claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `thoth setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.
...
- merge OpenClaw command approval patterns into Thoth `command_allowlist`
- migrate Thoth-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.thoth/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Thoth workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.thoth/tts/`
- archive non-secret docs that do not have a direct Thoth destination
...
1. Prefer the installed path under `~/.thoth/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Thoth-compatible secrets, currently:
...
Thoth CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Thoth. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Thoth workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Thoth skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Thoth unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Thoth targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Thoth destination. Unsupported auth blobs must still be reported as skipped.
...
- Thoth persona state imported
- Thoth memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.thoth/skills/openclaw-imports/`

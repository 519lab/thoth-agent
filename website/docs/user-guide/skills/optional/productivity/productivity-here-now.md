...
| | |
|---|---|
| Source | Optional — install with `thoth skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${THOTH_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${THOTH_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${THOTH_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client thoth
```
...
```bash
PUBLISH="${THOTH_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client thoth
```
...
```bash
DRIVE="${THOTH_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `thoth`)    |

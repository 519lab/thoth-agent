...
| | |
|---|---|
| Source | Optional — install with `thoth skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Thoth Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.thoth/.env`:
...
```bash
CANVAS="python $THOTH_HOME/skills/productivity/canvas/scripts/canvas_api.py"

...
| | |
|---|---|
| Source | Optional — install with `thoth skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Thoth Agent |
| License | MIT |
...
import upstream classes into Thoth itself.
...
```bash
mkdir -p ~/.thoth/cache/darwinian-evolver && cd ~/.thoth/cache/darwinian-evolver
...
```bash
cd ~/.thoth/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.thoth/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.thoth/skills/research/darwinian-evolver
DE_DIR=~/.thoth/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Thoth core.
   Custom driver scripts under `~/.thoth/skills/...` are user-side and fine.

...
| | |
|---|---|
| Source | Optional — install with `thoth skills install official/devops/watchers` |
...
All three live in `$THOTH_HOME/skills/devops/watchers/scripts/` once the skill is installed. Each reads `WATCHER_STATE_DIR` (defaults to `$THOTH_HOME/watcher-state/`) for its state file, keyed by the `--name` argument.
...
```bash
python $THOTH_HOME/skills/devops/watchers/scripts/watch_rss.py \
...
```bash
python $THOTH_HOME/skills/devops/watchers/scripts/watch_http_json.py \
...
Every watcher writes `$THOTH_HOME/watcher-state/<name>.json`. Inspect:

```bash
cat $THOTH_HOME/watcher-state/hn.json
```

Force a replay (next run treated as first poll):

```bash
rm $THOTH_HOME/watcher-state/hn.json
```
...
4. **Putting the state dir where the agent's sandbox can't write.** `$THOTH_HOME/watcher-state/` is always writable. Docker/Modal backends may not see arbitrary host paths.

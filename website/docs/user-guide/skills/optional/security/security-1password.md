...
| | |
|---|---|
| Source | Optional — install with `thoth skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Thoth Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Thoth terminal calls (desktop app flow only)
...
### Service Account (recommended for Thoth)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.thoth/.env` (the skill will prompt for this on first load).
...
## Thoth Execution Pattern (desktop app flow)

Thoth terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/thoth-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/thoth-op.sock"

...
Schedule tasks to run automatically with natural language or cron expressions. Thoth exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Thoth itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::warning
Cron-run sessions cannot recursively create more cron jobs. Thoth disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
thoth cron create "every 2h" "Check server status"
thoth cron create "every 1h" "Summarize new feed items" --skill blogwatcher
thoth cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Thoth will use the unified `cronjob` tool internally.
...
thoth cron create "every 1d at 09:00" \
...
thoth cron create "every 1d at 03:00" \
...
Use `--profile default` to explicitly pin to the root Thoth profile. The named profile must already exist; the scheduler refuses to create profiles on the fly. To clear a profile pin during `cron edit`, pass an empty string (`--profile ""` or `profile=""`) — the job reverts to running in whatever profile the scheduler itself is in.
...
```bash
thoth cron edit <job_id> --schedule "every 4h"
thoth cron edit <job_id> --prompt "Use the revised task"
thoth cron edit <job_id> --skill blogwatcher --skill maps
thoth cron edit <job_id> --add-skill maps
thoth cron edit <job_id> --remove-skill blogwatcher
thoth cron edit <job_id> --clear-skills
```
...
```bash
thoth cron list
thoth cron pause <job_id>
thoth cron resume <job_id>
thoth cron run <job_id>
thoth cron remove <job_id>
thoth cron status
thoth cron tick
```
...
```bash
thoth gateway install     # Install as a user service
sudo thoth gateway install --system   # Linux: boot-time system service for servers
thoth gateway             # Or run in foreground
...
1. loads jobs from `~/.thoth/cron/jobs.json`
...
A file lock at `~/.thoth/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.thoth/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.thoth/cron/output/`), but no message is sent to the delivery target.
...
Or set the `THOTH_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
`.sh` / `.bash` files run under `/bin/bash`; anything else under the current Python interpreter (`sys.executable`). Scripts must live in `~/.thoth/scripts/` (same sandboxing rule as the pre-run script gate).

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to Thoth directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Thoth will write the check script to `~/.thoth/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.thoth/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.thoth/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.thoth/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.thoth/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Thoth reads Job 1's most recent output from `~/.thoth/cron/output/{job1_id}/*.md`
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Thoth skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `thoth tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `thoth tools` cron-platform config wins; otherwise Thoth falls back to the built-in defaults. This matters for cost control: carrying `moa`, `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Thoth should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.thoth/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.thoth/scripts/.feed-changed.last"
...
:::tip
Thoth's own `~/.thoth/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/519lab/thoth-agent/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.thoth/cron/jobs.json`. Output from job runs is saved to `~/.thoth/cron/output/{job_id}/{timestamp}.md`.

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Thoth resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.

...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Thoth automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, Thoth auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
thoth auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
thoth auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
thoth auth add anthropic --type oauth
...
Run `thoth auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `thoth auth` | Interactive pool management wizard |
| `thoth auth list` | Show all pools and credentials |
| `thoth auth list <provider>` | Show a specific provider's pool |
| `thoth auth add <provider>` | Add a credential (prompts for type and key) |
| `thoth auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `thoth auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `thoth auth remove <provider> <index>` | Remove credential by 1-based index |
| `thoth auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `thoth auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `thoth model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via thoth model:
thoth auth list
# Shows:
...
thoth auth add Together.ai --api-key sk-together-second-key
```
...
Thoth automatically discovers credentials from multiple sources and seeds the pool on startup:
...
Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `thoth auth add`) are never auto-pruned.
...
2. **`thoth_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`thoth_cli/runtime_provider.py`** — Pool-aware credential resolution

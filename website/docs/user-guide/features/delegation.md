...
If a subagent times out having made **zero** API calls (usually: provider unreachable, auth failure, or tool-schema rejection), `delegate_task` writes a structured diagnostic to `~/.thoth/logs/subagent-timeout-<session>-<timestamp>.log` containing the subagent's config snapshot, credential-resolution trace, and any early error messages. Much easier to root-cause than the previous silent-timeout behavior.
:::

## Monitoring Running Subagents (`/agents`)

The `/agents` slash command (alias `/tasks`) prints a text summary of recursive `delegate_task` fan-out — running and recently-finished subagents, grouped by parent, with per-branch cost and token rollups.

...
The web dashboard is a browser-based UI for managing your Thoth Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.
...
```bash
# Custom port
thoth dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
thoth dashboard --host 0.0.0.0

# Start without opening browser
thoth dashboard --no-open
...
The Chat tab is intentionally off for a plain `thoth dashboard` launch. Start the dashboard with `thoth dashboard --tui` or set `THOTH_DASHBOARD_TUI=1` when you want the embedded browser chat pane.
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `thoth config set` and the gateway read from.
:::
...
Browse, search, and toggle skills and toolsets. Skills are loaded from `~/.thoth/skills/` and grouped by category.
...
This re-reads `~/.thoth/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `thoth_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `thoth update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `thoth dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Thoth Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Thoth Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |

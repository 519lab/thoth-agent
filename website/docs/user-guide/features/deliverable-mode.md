...
When Thoth Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
custom instructions in `~/.thoth/config.yaml` under `agent.custom_instructions`.
...
If you use Thoth's kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.thoth/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Thoth Agent's deliverable

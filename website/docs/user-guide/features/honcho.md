...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Thoth's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Thoth instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
thoth memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$THOTH_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `thoth` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Thoth syncs them back at session init.
...
The `thoth honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `thoth memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
thoth honcho status          # Connection status, config, and key settings
thoth honcho setup           # Redirects to `thoth memory setup`
thoth honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
thoth honcho peer            # Show or update peer names + dialectic reasoning level
thoth honcho mode            # Show or set recall mode (hybrid/context/tools)
thoth honcho tokens          # Show or set token budget for context and dialectic
thoth honcho identity        # Seed or show the AI peer's Honcho identity
thoth honcho sync            # Sync Honcho config to all existing profiles
thoth honcho peers           # Show peer identities across all profiles
thoth honcho sessions        # List known Honcho session mappings
thoth honcho map             # Map current directory to a Honcho session name
thoth honcho enable          # Enable Honcho for the active profile
thoth honcho disable         # Disable Honcho for the active profile
thoth honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `thoth honcho`

If you previously used the standalone `thoth honcho setup`:
...
No re-login or re-setup needed. Run `thoth memory setup` and select "honcho" — the wizard detects your existing config.

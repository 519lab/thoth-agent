...
description: "Connect Thoth Agent to external tool servers via MCP — and control exactly which MCP tools Thoth loads"
---

# MCP (Model Context Protocol)

MCP lets Thoth Agent connect to external tool servers so the agent can use tools that live outside Thoth itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Thoth to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Thoth tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Thoth to see
...
4. Ask Thoth to use the MCP-backed capability.
...
Thoth will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints Thoth connects to directly.
...
- you do not want Thoth spawning a local subprocess for that integration
...
| `enabled` | bool | If `false`, Thoth skips the server entirely |
...
For well-known MCP servers, `thoth mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
thoth mcp add codex --preset codex
```
...
You can pick any local name (`thoth mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How Thoth registers MCP tools

Thoth prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Thoth sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Thoth also registers utility tools around MCP resources and prompts:
...
- Thoth only registers resource utilities if the MCP session actually supports resource operations
- Thoth only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Thoth, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Thoth skips the server completely and does not even attempt a connection.
...
You can also separately disable Thoth-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Thoth does not create an empty runtime MCP toolset for that server.
...
Thoth discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Thoth when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Thoth receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Thoth does not blindly pass your full shell environment.
...
Then verify your config and restart Thoth.
...
Because Thoth now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Thoth may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Thoth via the `sampling/createMessage` protocol. This allows an MCP server to ask Thoth to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Thoth as an MCP server

In addition to connecting **to** MCP servers, Thoth can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Thoth's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Thoth
- You want a single MCP server that bridges to all of Thoth's connected messaging platforms at once
- You already have a running Thoth gateway with connected platforms
...
Add Thoth to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Thoth in a specific location:
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Thoth-specific channel browser:
...
The MCP server includes a live event bridge that polls Thoth's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
thoth mcp serve              # Normal mode
thoth mcp serve --verbose    # Debug logging on stderr
```
...
- The embedded `thoth mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Thoth, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).

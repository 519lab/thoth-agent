...
```bash
thoth chat --toolsets skills -q "What skills do you have?"
thoth chat --toolsets skills -q "Show me the axolotl skill"
```
...
Settings are stored under `skills.config` in your config.yaml. `thoth config migrate` prompts for unconfigured settings, and `thoth config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Thoth ignores it without errors. Useful for optional shared directories that may not be present on every machine.
...
- `description` (optional) — short text shown in `/bundles` and `thoth bundles list`.
...
thoth bundles create research

# Overwrite an existing bundle
thoth bundles create backend-dev --skill ... --force
...
- **Bundles work in every surface** — interactive CLI, dashboard chat, and every gateway platform (Telegram, Discord, Slack, …) — because dispatch is centralized in the same place as individual skill commands.
...
```bash
thoth skills browse                              # Browse all hub skills (official first)
thoth skills browse --source official            # Browse only official optional skills
thoth skills search kubernetes                   # Search all sources
thoth skills search react --source skills-sh     # Search the skills.sh directory
thoth skills search https://mintlify.com/docs --source well-known
thoth skills inspect openai/skills/k8s           # Preview before installing
thoth skills install openai/skills/k8s           # Install with security scan
thoth skills install official/security/1password
thoth skills install skills-sh/vercel-labs/json-render/json-render-react --force
thoth skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
thoth skills list --source hub                   # List hub-installed skills
thoth skills check                               # Check installed hub skills for upstream updates
thoth skills update                              # Reinstall hub skills with upstream changes when needed
thoth skills audit                               # Re-scan all hub skills for security
thoth skills uninstall k8s                       # Remove a hub skill
thoth skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
thoth skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
thoth skills publish skills/my-skill --to github --repo owner/repo
thoth skills snapshot export setup.json          # Export skill config
thoth skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Thoth. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `thoth skills search <query> --source skills-sh`. Thoth resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Thoth currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Thoth repository itself and install with builtin trust.
...
This is Vercel's public skills directory. Thoth can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
thoth skills search https://mintlify.com/docs --source well-known
thoth skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
thoth skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Thoth can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
thoth skills tap add myorg/skills-repo
```
...
- Thoth source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Thoth supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Thoth source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Thoth can search and convert agent entries from LobeHub's public catalog into installable Thoth skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Thoth source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

Thoth integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Thoth's browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Thoth source id: `browse-sh`
- Trust level: `community`

```bash
thoth skills search airbnb --source browse-sh
thoth skills inspect browse-sh/airbnb.com/search-listings-ddgioa
thoth skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Thoth fetches the URL, parses the YAML frontmatter, security-scans it, and installs.
...
```bash
thoth skills install https://sharethis.chat/SKILL.md
thoth skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
4. On non-interactive surfaces (the `/skills install` slash command in chat, gateway platforms, scripts), a clean error pointing at the `--name` override.

```bash
# Frontmatter has no name and the URL slug is unhelpful — supply one:
thoth skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `thoth skills update` re-fetches from the same URL automatically when you want to refresh.
...
`thoth skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Thoth | Always trusted |
...
```bash
thoth skills check          # Report which installed hub skills changed upstream
thoth skills update         # Reinstall only the skills with updates available
thoth skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Thoth users add with `thoth skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
Thoth discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
The `thoth skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `thoth skills tap list` shows the effective path per tap.
...
```bash
thoth skills install owner/repo/skills/my-workflow
```
...
thoth skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `THOTH_HOME`, so `thoth -p coder skills reset <name>` only affects that profile.
:::

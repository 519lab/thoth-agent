...
description: "Plugins shipped with Thoth Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `thoth memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `thoth plugins list` and the interactive `thoth plugins` UI), but none load until you explicitly enable them:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Thoth. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
thoth plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `thoth plugins enable <name>`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `THOTH_HOME` or `/tmp/thoth-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under THOTH_HOME | always | Never |
...
**State** — everything lives at `$THOTH_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `THOTH_HOME` or `/tmp/thoth-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `thoth plugins enable disk-cleanup` (or check the box in `thoth plugins`).

**Disabling again:** `thoth plugins disable disk-cleanup`.

### observability/langfuse

Traces Thoth turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Thoth's canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `thoth logs`.
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Thoth turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `THOTH_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Thoth session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `thoth chat` session lives under one Langfuse session.

**Verify:**

```bash
thoth plugins list                 # observability/langfuse should show "enabled"
thoth chat -q "hello"              # check the Langfuse UI for a "Thoth turn" trace
```
...
| `THOTH_LANGFUSE_RELEASE` | — | Release/version tag |
| `THOTH_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `THOTH_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `THOTH_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Thoth-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Thoth-prefixed wins when both are set.
...
**Disabling:** `thoth plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
```bash
thoth plugins enable google_meet
...
| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Thoth updates. |

...
See the [Plugins](/docs/user-guide/features/plugins) page for the general plugin system, and [Build a Thoth Plugin](/docs/guides/build-a-hermes-plugin) to write your own.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `HERMES_HOME` or `/tmp/thoth-*`, track it silently as `test` / `temp` / `cron-output`. |
...
**Safety** — cleanup only ever touches paths under `HERMES_HOME` or `/tmp/thoth-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.
...
Bundled plugins are written exactly like any other Thoth plugin — see [Build a Thoth Plugin](/docs/guides/build-a-hermes-plugin). The only differences are:

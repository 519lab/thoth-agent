...
Thoth Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
thoth memory setup      # interactive picker + configuration
thoth memory status     # check what's active
thoth memory off        # disable external provider
```

You can also select the active memory provider via `thoth plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.thoth/config.yaml`:
...
When a memory provider is active, Thoth automatically:
...
**Setup Wizard:**
```bash
thoth memory setup        # select "honcho" — runs the Honcho-specific post-setup
```
...
:::tip Migrating from `thoth honcho`
If you previously used `thoth honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Thoth profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Thoth profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Thoth profile. Host key `thoth` → default; `thoth.<profile>` for others. |
...
Scans every Thoth profile, creates host blocks for any profile without one, inherits settings from the default `thoth` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
thoth memory setup    # select "openviking"
# Or manually:
...
**Setup:**
```bash
thoth memory setup    # select "mem0"
# Or manually:
...
**Setup:**
```bash
thoth memory setup    # select "hindsight"
# Or manually:
thoth config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.thoth/.env
```
...
| `bank_id` | `thoth` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Thoth Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/519lab/thoth-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
thoth memory setup    # select "holographic"
# Or manually:
...
**Setup:**
```bash
thoth memory setup    # select "retaindb"
# Or manually:
...
# Then configure Thoth
thoth memory setup    # select "byterover"
# Or manually:
...
**Setup:**
```bash
thoth memory setup    # select "supermemory"
# Or manually:
...
| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `thoth` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |

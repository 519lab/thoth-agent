...
title: "Plugins"
description: "Extend Thoth with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Thoth has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
Drop a directory into `~/.thoth/plugins/` with a `plugin.yaml` and Python code:
...
Start Thoth — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.thoth/plugins/hello-world/plugin.yaml`**
...
**`~/.thoth/plugins/hello-world/__init__.py`**

```python
"""Minimal Thoth plugin — registers a tool and a hook."""
...
Drop both files into `~/.thoth/plugins/hello-world/`, restart Thoth, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.thoth/plugins/` are disabled by default. Enable them only for trusted repositories by setting `THOTH_ENABLE_PROJECT_PLUGINS=true` before starting Thoth.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `thoth <plugin> <subcommand>` |
...
Within each source, Thoth also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.thoth/plugins/model-providers/<name>/` and `~/.thoth/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

Sub-category plugins surface in `thoth plugins list` and the interactive `thoth plugins` UI under their **path-derived key** — e.g. `observability/langfuse`, `image_gen/openai`, `platforms/teams`. That key (not the bare manifest `name:`) is the value you pass to `thoth plugins enable …` / `disable …` and the string to add under `plugins.enabled` in `config.yaml`.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `thoth plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.thoth/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
thoth plugins                    # interactive toggle (space to check/uncheck)
thoth plugins enable <name>      # add to allow-list
thoth plugins disable <name>     # remove from allow-list + add to disabled
```

After `thoth plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

### What the allow-list does NOT gate

Several categories of plugin bypass `plugins.enabled` — they're part of Thoth's built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.thoth/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.thoth/plugins/`.

### Migration for existing users

When you upgrade to a version of Thoth that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.thoth/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)
...
Thoth has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.thoth/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Thoth also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:
...
| An **STT backend** (custom whisper binary, local ASR CLI) | Config-driven — set `THOTH_LOCAL_STT_COMMAND` env var to a shell template | [Voice Message Transcription (STT)](/docs/user-guide/features/tts#voice-message-transcription-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Thoth auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/docs/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `thoth skills tap add <repo>` | [Skills Hub](/docs/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/docs/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.thoth/hooks/<name>/` | [Event Hooks](/docs/user-guide/features/hooks#gateway-event-hooks) |
...
```bash
thoth plugins                                       # unified interactive UI
thoth plugins list                                  # table: enabled / disabled / not enabled
thoth plugins install user/repo                     # install from Git, then prompt Enable? [y/N]
thoth plugins install user/repo --enable            # install AND enable (no prompt)
thoth plugins install user/repo --no-enable         # install but leave disabled (no prompt)
thoth plugins update my-plugin                      # pull latest
thoth plugins remove my-plugin                      # uninstall
thoth plugins enable my-plugin                      # add to allow-list (flat plugin)
thoth plugins enable observability/langfuse         # add to allow-list (sub-category plugin)
thoth plugins disable my-plugin                     # remove from allow-list + add to disabled
```

For plugins under a sub-category directory (e.g. `plugins/observability/langfuse/`, `plugins/image_gen/openai/`), use the full `<category>/<plugin>` key — that's exactly what `thoth plugins list` shows in the **Name** column.

### Interactive UI

Running `thoth plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `thoth plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.

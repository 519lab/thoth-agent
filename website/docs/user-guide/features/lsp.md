...
Thoth runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Thoth
...
1. Thoth captures a baseline of current diagnostics for the file.
...
`node_modules` tree — Thoth installs both packages together when you
run `thoth lsp install typescript` or auto-install fires on first
use.

## CLI

```
thoth lsp status          # service state + per-server install status
thoth lsp list            # registry, optionally --installed-only
thoth lsp install <id>    # eagerly install one server
thoth lsp install-all     # try every server with a known recipe
thoth lsp restart         # tear down running clients
thoth lsp which <id>      # print resolved binary path
```

`thoth lsp status` is the best starting point — it shows which
...
shared location — the staging dir is fully Thoth-owned and is
...
Servers are kept alive for the life of the Thoth process. There's
...
**`Backend warnings` section in `thoth lsp status`**
...
When `thoth lsp status` shows a `Backend warnings` section, install
...
`~/.thoth/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.thoth/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `thoth lsp restart` to clear the set;

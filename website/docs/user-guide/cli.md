...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `HERMES_YOLO_MODE` is on (either `thoth --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
CLI sessions are stored in Thoth's SQLite state database under `~/.hermes/state.db`. The database keeps:

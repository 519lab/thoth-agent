...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Thoth Agent terminal interface — commands, keybindings, personalities, and more"
---
...
thoth chat --model "anthropic/claude-sonnet-4"

# With a specific provider
thoth chat --provider nous        # Use Nous Portal
thoth chat --provider openrouter  # Force OpenRouter
...
thoth chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
thoth --continue             # Resume the most recent CLI session (-c)
thoth --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
thoth chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
thoth -w                         # Interactive mode in worktree
thoth -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the Thoth CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Thoth CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `THOTH_YOLO_MODE` is on (either `thoth --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`thoth -c` or `thoth --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Thoth to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Thoth supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Thoth replies |
...
```bash
thoth -s thoth-agent-dev,github-auth
thoth chat -s github-pr-workflow -s github-auth
```

Thoth loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.thoth/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.thoth/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Thoth..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Thoth recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Thoth — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Thoth is working, Thoth prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Thoth to the background — just like any terminal process. The shell prints a confirmation:

```
Thoth Agent has been suspended. Run `fg` to bring Thoth Agent back.
```
...
Resume this session with:
  thoth --resume 20260225_143052_a1b2c3
...
```bash
thoth --continue                          # Resume the most recent CLI session
thoth -c                                  # Short form
thoth -c "my project"                     # Resume a named session (latest in lineage)
thoth --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
thoth --resume "refactoring auth"         # Resume by title
thoth -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `thoth sessions rename <id> <title>` from the command line. Use `thoth sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Thoth's SQLite state database under `~/.thoth/state.db`. The database keeps:
...
Thoth immediately confirms the task and gives you back the prompt:

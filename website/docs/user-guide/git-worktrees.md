...
title: "Git Worktrees"
description: "Run multiple Thoth agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Thoth Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Thoth so each session has a clean, isolated working directory.

## Why Use Worktrees with Thoth?

Thoth treats the **current working directory** as the project root:

- CLI: the directory where you run `thoth` or `thoth chat`
...
git worktree add ../repo-feature feature/thoth-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/thoth-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Thoth there:

```bash
cd ../repo-feature

# Start Thoth in the worktree
thoth
```
...
git worktree add ../repo-experiment-a feature/thoth-a
git worktree add ../repo-experiment-b feature/thoth-b
```
...
- Works on its own branch (`feature/thoth-a` vs `feature/thoth-b`).
...
- Thoth checkpoint data under `~/.thoth/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Thoth experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/thoth-checkpoints-docs`, `feature/thoth-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Thoth from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `thoth -w` (Automatic Worktree Mode)

Thoth has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `thoth/thoth-<hash>`).
...
```bash
thoth -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `thoth -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Thoth session its own clean checkout.

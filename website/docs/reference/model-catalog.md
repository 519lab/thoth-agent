...
```
https://thoth.519lab.com/docs/api/model-catalog.json
```
...
- **`version`** — integer schema version. Future schemas bump this; Thoth refuses manifests with versions it doesn't understand and falls back to the hardcoded snapshot.
- **`metadata`** — free-form dict at the manifest, provider, and model level. Any keys. Thoth ignores unknown fields, so you can annotate entries (`"tier": "paid"`, `"tags": [...]`, etc.) without coordinating a schema change.
...
| When | What happens |
|---|---|
| `/model` or `thoth model` | Fetches if disk cache is stale, else uses cache |
...
# editing OPENROUTER_MODELS or _PROVIDER_MODELS["nous"] in thoth_cli/models.py).

...
# or:
HERMES_INFERENCE_MODEL=anthropic/claude-sonnet-4.6 thoth -z "…"
```
...
- **Legacy `hermes.service` warning.** If Thoth detects a pre-rename `hermes.service` systemd unit (instead of the current `hermes-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.

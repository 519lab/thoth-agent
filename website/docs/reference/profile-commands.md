...
This page covers all commands related to [Thoth profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `thoth profile` without a subcommand shows help.
...
```bash
thoth profile use <name>
```

Sets `<name>` as the active profile. All subsequent `thoth` commands (without `-p`) will use this profile.
...
thoth profile use default
```

## `thoth profile create`

```bash
thoth profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `thoth profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-skills` marker into the profile so future `thoth update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. |
...
thoth profile create mybot

# Clone config only from current profile
thoth profile create work --clone

# Clone everything from current profile
thoth profile create backup --clone-all

# Clone config from a specific profile
thoth profile create work2 --clone --clone-from work
```

## `thoth profile describe`

```bash
thoth profile describe [<name>] [options]
```
...
thoth profile describe researcher

# Set it explicitly
thoth profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
thoth profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
thoth profile describe --all --auto
```

## `thoth profile delete`

```bash
thoth profile delete <name> [options]
```
...
```bash
thoth profile delete mybot
thoth profile delete mybot --yes
```
...
```bash
thoth profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Thoth home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
thoth profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Thoth installation.
...
```bash
thoth profile alias work
# Creates/updates ~/.local/bin/work

thoth profile alias work --name mywork
# Creates ~/.local/bin/mywork

thoth profile alias work --remove
# Removes the wrapper script
```

## `thoth profile rename`

```bash
thoth profile rename <old-name> <new-name>
```
...
## `thoth profile export`

```bash
thoth profile export <name> [options]
```
...
```bash
thoth profile export work
# Creates work.tar.gz in the current directory

thoth profile export work -o ./work-2026-03-29.tar.gz
```

## `thoth profile import`

```bash
thoth profile import <archive> [options]
```
...
```bash
thoth profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

thoth profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`thoth profile export` / `import` are still the right commands for
...
### `thoth profile install`

```bash
thoth profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `thoth -p telemetry`). |
...
thoth profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
thoth profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
thoth profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
thoth profile install ./telemetry/
```

### `thoth profile update`

```bash
thoth profile update <name> [--force-config] [--yes]
```
...
```bash
thoth profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Thoth version, author, env var requirements, the source URL/path, and
...
`thoth profile list` also shows the distribution name and version in a
`Distribution` column, and `thoth profile show <name>` / `delete <name>`
...
thoth profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
thoth profile install https://github.com/your-org/internal-assistant.git
```
...
`thoth_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Thoth version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Thoth can clone from.
3. Tell recipients to run `thoth profile install <your-repo-url>`.
...
## `thoth -p` / `thoth --profile`

```bash
thoth -p <name> <command> [options]
thoth --profile <name> <command> [options]
```

Global flag to run any Thoth command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
thoth -p work chat -q "Check the server status"
thoth --profile dev gateway start
thoth -p personal skills list
thoth -p work config edit
```

## `thoth completion`

```bash
thoth completion <shell>
```
...
- `thoth profile <TAB>` — subcommands (list, use, create, etc.)
- `thoth profile use <TAB>` — profile names
- `thoth -p <TAB>` — profile names

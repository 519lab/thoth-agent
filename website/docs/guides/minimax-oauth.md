...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Thoth Agent — no API key required"
---

# MiniMax OAuth

Thoth Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Thoth automatically refreshes your session.
...
# → Thoth opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.thoth/auth.json` and are refreshed automatically before each session.
...
```bash
thoth auth add minimax-cn --type oauth   # if OAuth is supported on your CN account
# or simpler:
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.thoth/.env
```
...
```bash
thoth auth add minimax-oauth --no-browser
```

Thoth will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Thoth implements a PKCE device-code flow against the MiniMax OAuth endpoints:

1. Thoth generates a PKCE verifier / challenge pair and a random state value.
...
4. Thoth polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.thoth/auth.json` under the `minimax-oauth` key.
...
```bash
thoth config set model MiniMax-M2.7
thoth config set provider minimax-oauth
```

## Configuration Reference

After login, `~/.thoth/config.yaml` will contain entries similar to:
...
```bash
thoth --provider minimax-oauth    # canonical
thoth --provider minimax-portal   # alias
thoth --provider minimax-global   # alias
thoth --provider minimax_oauth    # alias (underscore form)
```
...
Thoth refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Thoth marks the session as requiring re-login.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Thoth marks the refresh token as dead and quarantines it locally so it doesn't keep replaying the doomed exchange. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `thoth auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Thoth raises a timeout error.

**Fix:** re-run `thoth auth add minimax-oauth` (or `thoth model`). The flow starts fresh.

### State mismatch (possible CSRF)

Thoth detected that the `state` value returned by the authorization server does not match what it sent.
...
If `thoth` cannot open a browser window, use `--no-browser`:

```bash
thoth auth add minimax-oauth --no-browser
```

Thoth prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `thoth model` and select MiniMax (OAuth), or run `thoth auth add minimax-oauth`.
...
```bash
thoth auth remove minimax-oauth
```
...
- [thoth doctor](../reference/cli-commands.md)

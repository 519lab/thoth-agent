...
Copy-paste recipes for common automation patterns. Each template uses Thoth's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`thoth webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `thoth webhook subscribe`) |
...
```bash
thoth cron create "0 2 * * *" \
  "You are a project manager triaging the 519lab/thoth-agent GitHub repo.

1. Run: gh issue list --repo 519lab/thoth-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
thoth webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
thoth cron create "0 9 * * 1" \
  "Scan the 519lab/thoth-agent repo for documentation drift.

1. Run: gh pr list --repo 519lab/thoth-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (thoth_cli/commands.py, thoth_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (thoth_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
thoth webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
thoth webhook subscribe alert-triage \
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Thoth-Monitor/1.0"})
...
```bash
thoth cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Thoth Agent development." \
...
```bash
thoth webhook subscribe github-issues \
  --events "issues" \
...
```bash
thoth webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
thoth webhook subscribe stripe-payments \

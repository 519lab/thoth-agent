...
thoth teams-pipeline token-health --force-refresh
```
...
thoth teams-pipeline maintain-subscriptions --dry-run
```
...
```ini
[Unit]
Description=Thoth Teams pipeline subscription maintenance
...
```ini
[Unit]
Description=Run Thoth Teams pipeline subscription maintenance every 12 hours
...
```bash
thoth teams-pipeline subscriptions   # should show expirationDateTime advanced
thoth teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
thoth teams-pipeline list --status failed
thoth teams-pipeline show <job-id>
```

### Replay a stored job

```bash
thoth teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
thoth teams-pipeline fetch --meeting-id <meeting-id>
thoth teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `thoth teams-pipeline maintain-subscriptions --dry-run`
- inspect `thoth teams-pipeline list --status failed`
...
- run `thoth teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `thoth teams-pipeline run`
...
- [ ] `thoth teams-pipeline validate` returns an OK snapshot
- [ ] `thoth teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Thoth cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.

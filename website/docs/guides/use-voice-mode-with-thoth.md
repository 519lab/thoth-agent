---
sidebar_position: 8
title: "Use Voice Mode with Thoth"
description: "A practical guide to setting up and using Thoth voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Thoth
...
- you want Thoth sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Thoth.
...
## Step 1: make sure normal Thoth works first
...
```bash
pip install "thoth-agent[tts-premium]"
```
...
Thoth supports both local and cloud speech stacks.
...
### If you use `thoth setup`

If you choose NeuTTS in the setup wizard, Thoth checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Thoth transcribes and responds
...
- asking Thoth to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Thoth loop.

## Tuning CLI behavior

### Silence threshold

If Thoth starts/stops too aggressively, tune:
...
Thoth stays a normal chat bot, but can speak replies.
...
- you want Thoth to function like a portable research or ops assistant
...
Thoth joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Thoth detects speech boundaries
- transcripts are posted in the associated text channel
- Thoth responds in text and audio
...
1. get text Thoth working
2. install `thoth-agent[voice]`

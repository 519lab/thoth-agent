---
sidebar_position: 6
title: "Use MCP with Thoth"
description: "A practical guide to connecting MCP servers to Thoth Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Thoth

This guide shows how to actually use MCP with Thoth Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Thoth tool
- you want Thoth to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Thoth to internal APIs, databases, or company systems without modifying Thoth core

Do not use MCP when:
- a built-in Thoth tool already solves the job well
...
- Thoth remains the agent
- MCP servers contribute tools
- Thoth discovers those tools at startup or reload time
...
If you installed Thoth with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Thoth banner/status should show MCP integration when configured
- ask Thoth what tools it has available
...
## WSL2: bridge Thoth in WSL to Windows Chrome

This is the practical setup when:

- Thoth runs inside WSL2
...
In this setup, Thoth does **not** connect to Chrome directly. Instead:

- Thoth runs in WSL
- Thoth starts a local stdio MCP server
...
```text
Thoth (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Thoth stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Thoth core browser transport
...
```bash
thoth mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
thoth mcp test chrome-devtools-win
```

Then start a fresh Thoth session or run:
...
Once loaded, Thoth can use the MCP-prefixed browser tools directly. For example:
...
If Thoth runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Thoth from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Thoth from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Thoth:
...
2. Thoth-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Thoth will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Thoth to reason over a bounded workspace.
...
Now Thoth can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Thoth core.
...
Because Thoth now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.

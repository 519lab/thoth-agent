...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `thoth send`."
---

# Pipe Script Output to Messaging Platforms

`thoth send` is a small, scriptable CLI that pushes a message to any
messaging platform Thoth is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | thoth send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `thoth
...
thoth send --to telegram "deploy finished"

# Pipe in stdout from anything
echo "RAM 92%" | thoth send --to telegram:-1001234567890

# Send a file
thoth send --to discord:#ops --file /tmp/report.md

# Attach a subject/header line
thoth send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
thoth send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
thoth send --list

# Filter by platform
thoth send --list telegram
```
...
`thoth send` resolves the message body in this order:

1. **Positional argument** — `thoth send --to telegram "hi"`
2. **`--file PATH`** — `thoth send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | thoth send --to telegram`

When stdin is a TTY (no pipe), Thoth does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  thoth send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `thoth send` reuses your Thoth config, the same script works on
any host where Thoth is installed — no need to export bot tokens into
...
`thoth send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  thoth send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | thoth send \
...
```bash
./train.py --epochs 200 && \
  thoth send --to telegram "training done" || \
  thoth send --to telegram "training failed (exit $?)"
```
...
thoth send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(thoth send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `thoth send` Need the Gateway Running?
...
pointing at the gateway; start it with `thoth gateway start` and retry.
...
| Approach | Multi-platform | Reuses Thoth creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `thoth send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`thoth send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `thoth send`.
...
- [Gateway Internals](/docs/developer-guide/gateway-internals) —
  the delivery router that `thoth send` shares with cron delivery.

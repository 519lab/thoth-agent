...
description: "Run Thoth Agent directly on an Android phone with Termux"
---

# Thoth on Android with Termux

This is the tested path for running Thoth Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Thoth from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Thoth now ships a Termux-aware installer path:
...
- links `thoth` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `thoth` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/thoth" "$PREFIX/bin/thoth"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `thoth` command persist across new shells without re-activating the venv every time.
...
### `thoth doctor` says ripgrep or Node is missing

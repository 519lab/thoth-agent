...
sidebar_position: 2
title: "Installation"
description: "Install Thoth Agent on Linux, macOS, WSL2, native Windows (early beta), or Android via Termux"
---

# Installation

Get Thoth Agent up and running in under two minutes with the one-line installer.
...
2. Otherwise it downloads portable **PortableGit** (~50MB, from the official `git-for-windows` GitHub release) and unpacks it to `%LOCALAPPDATA%\thoth\git`.  No admin rights required.  Completely isolated — it won't interfere with any system Git install, broken or otherwise.  (On 32-bit Windows it falls back to MinGit because PortableGit ships only 64-bit and ARM64 assets; bash-dependent Thoth features won't work on 32-bit hosts.)

**Why not use winget?**  Earlier designs auto-installed Git via `winget install Git.Git`, but winget fails badly when a system Git install is in a partial or broken state (exactly when users need the installer to just work).  The portable Git approach sidesteps winget, the Windows installer registry, and any existing system Git entirely.  If the Thoth Git install itself ever breaks, `Remove-Item %LOCALAPPDATA%\thoth\git` and re-run the installer — no system impact, no uninstall drama.
...
Thoth now ships a Termux-aware installer path too:
...
- **CLI (`thoth chat`, `thoth setup`, `thoth gateway`, …)** — native, uses your default terminal
...
Set `THOTH_DISABLE_WINDOWS_UTF8=1` in your environment if you hit an encoding-related bug and want to fall back to the legacy cp1252 stdio path (useful for bisecting).
:::

### What the Installer Does

The installer handles everything automatically — all dependencies (Python, Node.js, ripgrep, ffmpeg), the repo clone, virtual environment, global `thoth` command setup, and LLM provider configuration. By the end, you're ready to chat.
...
thoth gateway setup  # Set up messaging platforms
thoth config set     # Set individual config values
thoth setup          # Or run the full setup wizard to configure everything at once
```
...
2. **As the unprivileged service user**, run the regular installer. It will detect the missing sudo, skip `--with-deps`, and install Chromium into the user's local Playwright cache:
   ...
   ```bash
   curl -fsSL https://raw.githubusercontent.com/519lab/thoth-agent/main/scripts/install.sh | bash -s -- --skip-browser
   ```

3. **Make `thoth` available to the service user's shells.** The installer writes the launcher to `~/.local/bin/thoth`. System service accounts often have a minimal PATH that doesn't include `~/.local/bin`. Either add it to the user's environment, or symlink the launcher into a system location:
   ```bash
...
For more diagnostics, run `thoth doctor` — it will tell you exactly what's missing and how to fix it.

## Install method auto-detection

Thoth auto-detects whether it was installed via `pip`, the git installer, or Homebrew, and `thoth update` prints the matching update command for that path. There's no env var to set — the detection is based on the install layout (Python site-packages, `~/.thoth/thoth-agent/`, or Homebrew prefix). `thoth doctor` also surfaces the detected method under its environment summary.

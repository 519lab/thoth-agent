...
description: "Install and deploy Thoth Agent with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---

# Nix & NixOS Setup

Thoth Agent ships a Nix flake with three levels of integration:
...
**For non-NixOS users**, this only changes the install step. Everything after (`thoth setup`, `thoth gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage thoth the same way you manage any other NixOS service.
:::
...
nix run github:519lab/thoth-agent -- setup
nix run github:519lab/thoth-agent -- chat

# Or install persistently
nix profile install github:519lab/thoth-agent
thoth setup
thoth chat
```
...
That's it. `nixos-rebuild switch` creates the `thoth` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o thoth /dev/stdin /var/lib/thoth/env
```
...
Setting `addToSystemPackages = true` does two things: puts the `thoth` CLI on your system PATH **and** sets `HERMES_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `thoth` in your shell creates a separate `~/.hermes/` directory.
:::

### Container-aware CLI

:::info
When `container.enable = true` and `addToSystemPackages = true`, **every** `thoth` command on the host automatically routes into the managed container. This means your interactive CLI session runs inside the same environment as the gateway service — with access to all container-installed packages and tools.
...
Users listed in `hostUsers` are automatically added to the `thoth` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo thoth chat` manually.
:::
...
thoth version
thoth config       # shows the generated config
```
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, Thoth defaults to OpenRouter.
:::
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$HERMES_HOME/.env` at activation time (`nixos-rebuild switch`). Thoth reads this file on every startup, so changes take effect with a `systemctl restart hermes-agent` — no container recreation needed.
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). Thoth looks for specific filenames by convention:

- **`USER.md`** — context about the user the agent is interacting with.
- Any other files you place here are visible to the agent as workspace files.

The agent identity file is separate: Thoth loads its primary `SOUL.md` from `$HERMES_HOME/SOUL.md`, which in the NixOS module is `${services.hermes-agent.stateDir}/.hermes/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.
...
Set `auth = "oauth"` for servers using OAuth 2.1. Thoth implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.
...
The first OAuth authorization requires a browser-based consent flow. In a headless deployment, Thoth prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u thoth` (native):
...
# Native mode
sudo -u thoth HERMES_HOME=/var/lib/hermes/.hermes \
  thoth mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
When thoth runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `thoth setup` | Config is declarative — edit `settings` in your Nix config |
| `thoth config edit` | Config is generated from `settings` |
| `thoth config set <key> <value>` | Config is generated from `settings` |
| `thoth gateway install` | The systemd service is managed by NixOS |
| `thoth gateway uninstall` | The systemd service is managed by NixOS |

This prevents drift between what Nix declares and what's on disk. Detection uses two signals:

1. **`HERMES_MANAGED=true`** environment variable — set by the systemd service, visible to the gateway process
2. **`.managed` marker file** in `HERMES_HOME` — set by the activation script, visible to interactive shells (e.g., `docker exec -it hermes-agent thoth config set ...` is also blocked)
...
When container mode is enabled, thoth runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the thoth package itself do **not** trigger recreation.
...
The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current thoth package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `thoth plugins install` needed.
...
Plugins are symlinked into `$HERMES_HOME/plugins/` at activation time. Thoth discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
The package's `site-packages` is added to PYTHONPATH in the thoth wrapper. `importlib.metadata` discovers the entry point at session start.
...
:::note
A build-time collision check prevents plugin packages from shadowing core thoth dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
| `stateDir` | `str` | `"/var/lib/thoth"` | State directory (`HERMES_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory (`MESSAGING_CWD`) |
| `addToSystemPackages` | `bool` | `false` | Add `thoth` CLI to system PATH and set `HERMES_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `thoth gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the thoth user's per-user profile so terminal commands, skills, and cron jobs all see them |
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Thoth binary + all Nix deps |
| `/data` | `/var/lib/thoth` | `rw` | All state, config, workspace |
| `/home/thoth` | `${stateDir}/home` | `rw` | Persistent agent home — `pip install --user`, tool caches |

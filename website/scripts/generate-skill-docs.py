...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/519lab/thoth-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    thoth_meta = (fm.get("metadata") or {}).get("thoth") or {}
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `thoth skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Thoth Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Thoth ships with a large built-in skill library copied into `~/.thoth/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Thoth also syncs bundled skills on `thoth update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.thoth/skills/` tree, it is still shipped with Thoth; restore it with `thoth skills reset <name> --restore`.",
        "",

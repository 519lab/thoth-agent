...
thoth kanban watch --tenant <project-slug>

# Snapshot of the board
thoth kanban list --tenant <project-slug>
thoth kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
thoth kanban stats --tenant <project-slug>

# Visual dashboard (browser)
thoth dashboard

# Inspect a specific task (includes comments + events)
thoth kanban show <task-id>

# Follow a single task's event stream
thoth kanban tail <task-id>
```

Verify available subcommands with `thoth kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `thoth kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `thoth kanban block <id>` then `thoth kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `thoth kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `thoth kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `thoth profile list` (profile exists?), `thoth status` (gateway/dispatcher up?), and verify tenant. |
...
thoth kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
thoth kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(thoth kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
thoth kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
thoth kanban block <task-id>
thoth kanban archive <task-id>

# Diagnose what happened
thoth kanban show <task-id>      # task body, comments, recent events
thoth kanban tail <task-id>      # follow the live event stream
thoth kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `thoth kanban create` for the director
...
while true; do
    clear
    thoth kanban list --tenant <slug>
    echo "---"
    thoth kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `thoth kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `thoth profile list` and start

---
name: agentmail
description: Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. thoth-agent@agentmail.to).
version: 1.0.0
...
Add to `~/.thoth/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
```
1. create_inbox (username: "thoth-outreach")

...
Usage (via cron with --no-agent):
    ...
    thoth cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$THOTH_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Thoth-Watcher/1.0"})

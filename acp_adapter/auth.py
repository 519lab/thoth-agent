"""ACP auth helpers — detect and advertise Thoth authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "thoth-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Thoth runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Thoth can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Thoth.
    ...
    not have Thoth provider credentials configured yet, so Thoth always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Thoth using the currently configured "

...
React + Ink terminal UI for Thoth. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `THOTH_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.thoth/.thoth_history` or under `THOTH_HOME`.
...
```text
ui-tui/
  packages/thoth-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      thoth-ink.d.ts    type declarations for @thoth/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ThothCLI subprocess for slash commands
```

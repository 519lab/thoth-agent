import type { RunExternalProcess } from '@thoth/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`thoth ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching thoth: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`thoth ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })

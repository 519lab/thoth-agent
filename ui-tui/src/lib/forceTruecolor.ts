...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.THOTH_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Thoth on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via THOTH_TUI_TRUECOLOR=1.

...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with thoth-agent)
2. User plugins: ``$THOTH_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$THOTH_HOME/plugins/model-providers/`` can override
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple THOTH_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_thoth_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$THOTH_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with thoth-agent.
    ...
    # 2. User plugins — under $THOTH_HOME/plugins/model-providers/<name>/.

...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``thoth tools`` config for the ``cron`` platform.
    ...
    if per_job:
        return per_job
    try:
        from thoth_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _job_profile_context(job_id: str, profile: Optional[str]):
    """Temporarily run a job under a specific Thoth profile.
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("THOTH_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid THOTH_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within THOTH_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against THOTH_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    # escape — scripts MUST reside within THOTH_HOME/scripts/.
    try:
    ...
    run_env["THOTH_HOME"] = str(_get_thoth_home())
    try:
...
def _run_job_impl(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        now_iso = _thoth_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_thoth_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_thoth_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["THOTH_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed THOTH_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads THOTH_SESSION_PLATFORM
    #     and THOTH_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via THOTH_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     THOTH_SESSION_PLATFORM == "telegram".
    ...
    #     send_message gate read THOTH_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the THOTH_CRON_AUTO_DELIVER_* vars set
    # below, so clearing THOTH_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "THOTH_CRON_AUTO_DELIVER_PLATFORM",
        "THOTH_CRON_AUTO_DELIVER_CHAT_ID",
        "THOTH_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        if delivery_target:
            _VAR_MAP["THOTH_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["THOTH_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["THOTH_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        model = job.get("model") or os.getenv("THOTH_MODEL") or ""
        ...
        prefill_file = os.getenv("THOTH_PREFILL_MESSAGES_FILE", "") or _cfg.get("prefill_messages_file", "")
        if prefill_file:
        ...
        try:
            # Do not inject THOTH_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # THOTH_HOME. When a workdir is configured, also inject project
        ...
        # override via THOTH_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("THOTH_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid THOTH_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_thoth_now().strftime('%Y-%m-%d %H:%M:%S')}
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _thoth_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _thoth_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set THOTH_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("THOTH_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid THOTH_CRON_MAX_PARALLEL value; defaulting to unbounded")

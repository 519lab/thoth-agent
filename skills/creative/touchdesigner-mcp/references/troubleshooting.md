...
## 5. Thoth Configuration

### Config location

`$THOTH_HOME/config.yaml` (defaults to `~/.hermes/config.yaml` when `THOTH_HOME` is unset)
...
Restart the Thoth session for changes to take effect. The MCP connection is

...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.thoth/.env ] && grep -q "^GITHUB_TOKEN=" ~/.thoth/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.thoth/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Thoth Agent\",
    \"comments\": [
...
```bash
source "${THOTH_HOME:-$HOME/.thoth}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Thoth Agent. Code looks clean — good test coverage, no security concerns."

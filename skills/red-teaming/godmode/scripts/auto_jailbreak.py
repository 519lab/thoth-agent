...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("THOTH_HOME", os.path.expanduser("~/.thoth")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("THOTH_HOME", Path.home() / ".thoth")) / "skills" / "red-teaming" / "godmode"
...
THOTH_HOME = Path(os.getenv("THOTH_HOME", Path.home() / ".thoth"))
CONFIG_PATH = THOTH_HOME / "config.yaml"
PREFILL_PATH = THOTH_HOME / "prefill.json"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Thoth config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.thoth/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Thoth for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Thoth for changes to take effect.")

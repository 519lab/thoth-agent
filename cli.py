#!/usr/bin/env python3
"""
Thoth Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Thoth Agent, inspired by Claude Code.
...
os.environ["THOTH_QUIET"] = "1"  # Our own modules
...
try:
    from thoth_cli.pt_input_extras import install_shift_enter_alias, install_ctrl_enter_alias
...
from thoth_cli.banner import _format_context_length, format_banner_version_label
...
load_thoth_dotenv(thoth_home=_thoth_home, project_env=_project_env)
...
def load_cli_config() -> Dict[str, Any]:
    ...
    """
    # Check user config first ({THOTH_HOME}/config.yaml)
    user_config_path = _thoth_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("THOTH_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Thoth, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Thoth - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and ThothCLI.__init__ checks "default" first.
            ...
            # location is model.provider (written by `thoth model`).
    ...
    # Normalize config key: the new config system (thoth_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && thoth` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _THOTH_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_THOTH_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["THOTH_REDACT_SECRETS"] = str(redact).lower()
...
try:
    from thoth_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``thoth --help`` paths inside the chat command flow).  See
...
from thoth_cli.banner import build_welcome_banner
from thoth_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from thoth_cli.callbacks import prompt_for_secret
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run thoth -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"thoth-{short_id}"
    branch_name = f"thoth/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`thoth_cli.config.load_config` (the authoritative loader that
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`thoth_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``thoth/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("thoth-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``thoth/thoth-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``thoth -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("thoth/thoth-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. THOTH_LIGHT / THOTH_TUI_LIGHT env (true/false) — explicit override
#   2. THOTH_TUI_THEME=light|dark — explicit theme
#   3. THOTH_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Thoth assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("THOTH_LIGHT", "THOTH_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("THOTH_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("THOTH_TUI_BACKGROUND") or ""
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Thoth output it's common to display cron schedules verbatim.
    #
...
from thoth_constants import is_termux as _is_termux_environment
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``ThothCLI._busy_command()``.
        """
        yield self

# ASCII Art - THOTH-AGENT wordmark (full width, single line - requires ~95 char terminal)
THOTH_AGENT_LOGO = """[bold #FFD700]████████╗██╗  ██╗ ██████╗ ████████╗██╗  ██╗     █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]╚══██╔══╝██║  ██║██╔═══██╗╚══██╔══╝██║  ██║    ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]   ██║   ███████║██║   ██║   ██║   ███████║    ███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]   ██║   ██╔══██║██║   ██║   ██║   ██╔══██║    ██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]   ██║   ██║  ██║╚██████╔╝   ██║   ██║  ██║    ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# ASCII Art - Thoth, ibis-headed god of writing, bearing the was-staff and ankh (fits in left panel)
THOTH_IBIS = """[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣀⣀⡀[/]
[#FFD400]⠀⠀⠀⠀⠀⠀⠀⠀⣀⡴⠾⢿⣿⣿⣿⣦⣄[/]
[#FFD100]⠀⠀⠀⡀⠀⠀⢠⠞⠁⠀⣀⣼⣿⣿⣿⣿⣿⣦[/]
[#FFCE00]⠀⠀⠘⡆⠀⠀⢈⣤⣴⣾⣿⣿⣿⣿⣿⣿⣿⣿⡆[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ THOTH - AI Agent Framework"
        tiny_line = "⚕ THOTH"
    else:
        agent_name = _skin.get_branding("agent_name", "Thoth Agent") if _skin else "Thoth Agent"
    ...
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- 519lab[/]\n"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from thoth_cli.plugins import get_plugin_commands
...
class ThothCLI:
    """
    Interactive CLI for the Thoth Agent.
    ...
    def __init__(
        ...
        self.requested_provider = (
            ...
            or os.getenv("THOTH_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("THOTH_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("THOTH_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `thoth chat --ignore-rules` in thoth_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("THOTH_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("THOTH_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all Thoth processes for this THOTH_HOME.
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from thoth_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            yolo_active = bool(os.getenv("THOTH_YOLO_MODE"))
            if width < 52:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Thoth'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if resolved_provider == "copilot":
            try:
                from thoth_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from thoth_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from thoth_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ Thoth")
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - ThothCLI._status_bar_display_width(label)
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: thoth setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: thoth setup")
            return False
        ...
        # `thoth chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `thoth auth add openai-codex`
        # without `thoth model`), fall back to the provider's first catalog
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``thoth doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``thoth doctor`` so the banner stays
        small.
        """
        try:
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len <= 8192:
            ...
            self._console_print(
                "[dim]   Thoth needs 16k–32k minimum. Tool schemas + system prompt alone use ~4k–8k.[/]"
            )
        ...
        from thoth_cli.model_switch import is_nous_hermes_non_agentic
        ...
        if is_nous_hermes_non_agentic(model_name):
            ...
            self._console_print(
                ...
                "designed for use with Thoth Agent.[/]"
            )
            self._console_print(
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Thoth: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Thoth: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: thoth --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Thoth config/state.
    ...
    def _handle_paste_command(self):
        ...
        from thoth_cli.clipboard import has_clipboard_image
    ...
    def show_help(self):
        ...
        from thoth_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from thoth_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Thoth!{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in thoth_cli/colors.py
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from thoth_cli.main import _relative_time
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        # Resolve title or ID
        from thoth_cli.main import _resolve_session_by_name_or_id
    ...
    def save_conversation(self):
        ...
        DB, so the live session remains resumable via ``thoth --resume <id>``
        ...
        saved_dir = get_thoth_home() / "sessions" / "saved"
        try:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from thoth_cli.curses_ui import curses_single_select
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `thoth model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from thoth_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from thoth_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from thoth_cli.model_switch import switch_model, parse_model_flags
        ...
        # clobber disk config.
        from thoth_cli.inventory import build_models_payload, load_picker_context
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  — Thoth default (chat_completions)
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to thoth_cli.curator so the CLI and the `thoth curator`
        ...
        try:
            from thoth_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to thoth_cli.skills_hub."""
        from thoth_cli.skills_hub import handle_skills_slash
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in thoth_cli/commands.py instead of touching every dispatch site.
        from thoth_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            from thoth_cli.config import reload_env
            count = reload_env()
            ...
            try:
                from thoth_cli.plugins import get_plugin_manager
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from thoth_cli.commands import COMMANDS
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``thoth bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: thoth bundles create "
        ...
        _cprint(
            ...
            f"Manage with `thoth bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Thoth-managed isolated debug "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Thoth keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from thoth_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from thoth_cli.config import load_config
        from thoth_cli.colors import Colors as _Colors
    ...
    def _toggle_yolo(self):
        ...
        current = is_truthy_value(os.environ.get("THOTH_YOLO_MODE"))
        if current:
            os.environ.pop("THOTH_YOLO_MODE", None)
            _cprint(
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Thoth is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Thoth is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Thoth is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from thoth_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _handle_debug_command(self):
        ...
        from thoth_cli.debug import run_debug_share
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Thoth Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``thoth update`` so the user sees update output directly and gets
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Thoth Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Thoth Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Thoth Agent",
            detail="This will exit the current session and run `thoth update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from thoth_cli.config import get_config_path as _get_config_path
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_thoth_home / "config.yaml", TOOL_PROGRESS_FLAG)
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _thoth_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            try:
                from thoth_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from thoth_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from thoth_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Thoth Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Thoth Agent! Type your message or /help for commands."
        ...
        try:
            _redact_raw = os.getenv("THOTH_REDACT_SECRETS", "true")
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    "[bold red]⚠  Secret redaction is DISABLED[/] "
                    f"(THOTH_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→Thoth migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from thoth_cli.config import get_config_path as _get_cfg_path_resid
        ...
        try:
            ...
            _THOTH_RESIDUE_FLAG = "hermes_residue_import"
            if not is_seen(self.config, _THOTH_RESIDUE_FLAG) and detect_hermes_residue():
                try:
                ...
                try:
                    ...
                    mark_seen(_get_cfg_path_hres(), _THOTH_RESIDUE_FLAG)
                except Exception:
                    pass  # best-effort — banner will fire again next session
        except Exception:
            pass  # banner is non-critical — never break startup
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_thoth_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Thoth binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from thoth_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Thoth Agent")
        ...
        # Dynamic prompt: shows Thoth symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Thoth needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Thoth needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Thoth needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = ThothCLI._compute_model_picker_viewport(
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``THOTH_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("THOTH_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the thoth process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: thoth setup"
            )
            _run_cleanup()
...
def main(
    ...
    """
    Thoth Agent CLI - Interactive AI Assistant
    ...
    # Phase 0: initialise PG pool (idempotent; raises if THOTH_PG_DSN unset).
    try:
    ...
    if gateway:
        ...
        print("Starting Thoth Gateway (messaging platforms)...")
    ...
    # Default to thoth-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    # mode registers its own inside ThothCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  THOTH_SIGTERM_GRACE overrides the 1.5 s
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "Thoth" box, no tool-gen
            ...
            # Single-query mode (`thoth chat -q "…"`): skip the welcome
